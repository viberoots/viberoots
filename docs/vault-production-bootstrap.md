...
9. Add another `Hardcoded claim` mapper for `repository` with value
   `kiltyj/common`, JSON type `String`, and access token inclusion
   enabled.
...
deploy-vault-jwt \
  ...
  --expect-claim repository=kiltyj/common \
  --print-claims
```
...
vault write auth/jwt/role/deploy-pleomino-read \
  ...
  bound_claims='{"azp":"deployment-runner","deployment_environment":"mini","repository":"kiltyj/common"}' \
  user_claim="sub" \
