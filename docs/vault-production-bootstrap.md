...
mode, do not import `/srv/viberoots/...` as an absolute path from
`configuration.nix`. Also avoid making all of `/srv/viberoots` a flake input,
...
from the checkout that lives at `/srv/viberoots`:
