...
- the installer creates platform-state, runtime, and records locations under
  `/var/lib/deployment-host`, but treats that backend data as persistent host
  data
...
`/var/lib/deployment-host` is the canonical backend root for shared-host state,
runtime materialization, records, and the deployment service home on every host.
The generated `deployment-host-managed.nix` imports the shared-host module via
`"${deploymentModulesRoot}/nixos-shared-host-module.nix"`, so flake-based hosts
must expose `/srv/common/build-tools/tools/nix` as `deploymentModulesRoot`
before running `nixos-rebuild switch --flake`.
