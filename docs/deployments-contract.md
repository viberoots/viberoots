...
- Protected/shared artifact challenge issuance must authenticate the service client, resolve the same deployment authorization boundary used by final submit without consuming the auth session, reject unauthorized requested operations before persistence, and bind the challenge to the authorized principal, proof key id, deployment, operation, provider target, source, publish behavior, idempotency intent, and expected artifact identity.
