...
- For public repo-level protected/shared `cloudflare-pages` mutation, missing or invalid context-selected `controlPlanes.<name>` service-client metadata is a fail-closed configuration error. `--control-plane-url` and `VBR_DEPLOY_CONTROL_PLANE_URL` are reserved for commands without deployment context or explicit reviewed overrides, and mixing the service-routed path with local-only flags such as `--records-root` or `--control-plane-database-url` is out of contract.
...
- Protected/shared artifact challenge issuance must authenticate the required hosted service bearer token, resolve the same deployment authorization boundary used by final submit without consuming the auth session, reject unauthorized requested operations before persistence, and bind the challenge to the authorized principal, proof key id, deployment, operation, provider target, source, publish behavior, idempotency intent, expected artifact identities, and the finalized staged artifact reference used by final submit.
- The proof-key registry must reject unknown, disabled, unreviewed, unassigned, or algorithm-mismatched key ids at challenge issuance and again at final submit.
- Protected/shared challenged artifact submissions must accept through one backend transaction that resolves idempotent retry before consuming a one-time challenge, then commits challenge consumption, admitted artifact-binding provenance, the frozen execution snapshot, the accepted submission, and worker queue intent together.
- Final submit must recompute the authorization boundary and fail closed before artifact admission, snapshot creation, queueing, worker execution, or provider mutation when the active token, authorized principal, proof key id, deployment envelope, source/publish selection, idempotency semantics, or expected artifact identities differ from the challenge binding.
- Retrying the same idempotency key with the same challenged request fingerprint returns the accepted submission even after the challenge is consumed. Reusing the key with a different challenge, proof, expected identity, source, staged reference, or canonical envelope fails as an idempotency conflict, and challenge replay without the matching accepted idempotency fingerprint fails closed.
- Rejected protected/shared staged uploads must be cleaned by the service, or represented by a bounded redacted janitor record if cleanup fails. Client flags such as `--retain-remote-artifact` must not preserve rejected protected/shared auth, challenge, proof, identity, admission, or queue-precondition failures.
- Accepted challenged submissions must expose only redacted artifact-binding provenance: challenge id, authenticated principal and proof key id, proof algorithm, canonical envelope fingerprint, expected identity, recomputed admitted identity, verification decision/time, and an admitted artifact reference. They must not expose proof secrets, proof MACs, challenge nonces, bearer tokens, or full staged artifact paths.
- Protected/shared service-client loopback HTTP is a local-fixture transport only and requires an explicit reviewed marker such as `VBR_DEPLOY_LOCAL_FIXTURE_SERVICE=1`; manifest and flag resolution must not grant implicit localhost exceptions.
- The reviewed hosted protected/shared service must fail closed when the bearer-token requirement is missing; tokenless startup or request handling is valid only for explicit local fixture mode.
...
- Reviewed protected/shared submit, status, and run-action responses, including nested approval summaries, must carry backend-native identifiers and lifecycle state, not routine `submissionPath`, `resultRecordPath`, `recordPath`, `executionSnapshotPath`, or similar filesystem-mirror fields.
