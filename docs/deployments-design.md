...
| `kubernetes`        | `service`<br>`third-party-service` | reviewed for `service`<br>`third-party-service`; explicit `rollout_policy` required                                                 | not reviewed                                 | `all_at_once`<br>`ordered_best_effort` | `all_at_once`        | `helm-release`                                                      | `terraform-stack`<br>`cdktf-stack`     | none                                          |
| `vercel`            | `ssr-webapp`                       | single-component only                                                                                                               | not reviewed                                 | `all_at_once`                          | `all_at_once`        | `vercel-prebuilt`                                                   | none                                   | none                                          |
