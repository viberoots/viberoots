> Note: C++ provider sync is a no-op. This document is retained for historical context only. The generator described below is not used by current builds; new work should rely on `nixpkg:` labels and the planner. See `build-tools/docs/cpp-provider-sync-migration.md` and `build-tools/tools/buck/inspect-cpp-attrs.ts`.
...
- Inputs to rebuild/invalidate: `build-tools/tools/buck/graph.json`, `patches/cpp/*.patch`, `build-tools/tools/nix/overlays/cpp-patches.nix`, `flake.lock`.
...
Generator: build-tools/tools/buck/providers/cpp.ts (invoked by sync CLI)

- Inputs
  - Graph: `build-tools/tools/buck/graph.json` (must exist).
  - Patches dir: `patches/cpp` (flat, optional).
  - Overlay file: `build-tools/tools/nix/overlays/cpp-patches.nix` (optional).
...
- Local (developer):
  - `patch-pkg apply cpp pkgs.<attr>` → writes patch file → `node build-tools/tools/buck/sync-providers.ts --lang=cpp` → `node build-tools/tools/buck/gen-auto-map.ts` → build
- CI stages (ordered):
  1. Export Graph → `build-tools/tools/buck/graph.json`
  2. Sync C++ Providers → `node build-tools/tools/buck/sync-providers.ts --lang=cpp`
  3. Generate auto_map → `node build-tools/tools/buck/gen-auto-map.ts`
...
- Keep the generator language-agnostic by extracting a minimal core in `build-tools/tools/buck/providers/index.ts`:
...
2. Create `build-tools/tools/buck/providers/cpp.ts` implementing the steps above.
3. Update `build-tools/tools/buck/providers/index.ts` to call the new C++ sync.
4. Wire into `patch-pkg apply cpp` and CI stages.
5. Add unit + e2e tests under `build-tools/tools/tests/cpp/` and `build-tools/tools/tests/dev/`.
