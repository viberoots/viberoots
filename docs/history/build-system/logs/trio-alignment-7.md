...
- Make `build-tools/tools/nix/lib/lang-helpers.nix` the authoritative source for:
...
- Deprecate `build-tools/tools/nix/dev-overrides.nix`:
...
- Retain `patchesMapFromDir` in `build-tools/tools/nix/lib/lang-helpers.nix` as canonical.
- Update `build-tools/tools/nix/templates/*.nix` to use the canonical helper.
- Deprecate `build-tools/tools/nix/templates-common.nix`:
...
- Move `shortHash` to a single definition:
  - Keep it in `build-tools/tools/lib/providers.ts`.
  - Update `build-tools/tools/lib/provider-names.ts` to import it; remove duplicate implementation.
- Adopt `build-tools/tools/lib/cli.ts` across remaining bespoke callers:
  - Migrate `build-tools/tools/patch/patch-pkg.ts` (replace local parser with `getFlagStr/getFlagBool/getFlagList` where applicable).
- Tests:
  - Add small unit tests for `shortHash` import path.
  - Ensure `patch-pkg` flag behavior remains identical (argv precedence).
- Docs:
  - Reference `build-tools/tools/lib/cli.ts` as the canonical way to read flags in zx tools.
...
- Add `maybeAssumeUnchanged(file: string)` to `build-tools/tools/lib/fs-helpers.ts`.
- Update `build-tools/tools/buck/gen-auto-map.ts` (and any other script that sets the hint) to call the helper.
