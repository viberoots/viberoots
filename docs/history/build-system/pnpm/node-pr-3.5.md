...
This proposal closes the remaining gaps to fully implement and validate PR 3.5 from `docs/history/designs/legacy/nix-node-test.md`: migrate Node scaffolding templates to rely on the hermetic `nix_node_test` external runner with default discovery, avoid legacy test shims, and add focused zx tests.
