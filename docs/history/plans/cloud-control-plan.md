...
[Cloud Control Plane Design](../designs/cloud-control-design.md).
...
- Update `docs/history/designs/cloud-control-design.md` only if the contract changes.
...
- Add a managed Postgres conformance checklist to `docs/history/designs/cloud-control-design.md` or a linked runtime
  operations doc.
...
- Update `docs/history/designs/control-plane-containerization.md` and runtime config docs with artifact-store
  settings.
...
- Update `docs/history/designs/control-plane-containerization.md`.
...
- Add managed dependency setup docs linked from `docs/history/designs/cloud-control-design.md`.
...
- Update `docs/history/designs/cloud-control-design.md` to point to the setup command as the operator entrypoint.
...
- Update `docs/history/designs/cloud-control-design.md` with the concrete capability catalog.
...
## PR-18: Executable provider-capability hook dispatch

### 1. Intent

Move provider-capability definitions from reviewed static declarations to executable, fixture-tested
hook contracts that can run preview, apply, evidence capture, smoke, and rollback through the
deployment control-plane tooling.

### 2. Scope of changes

- Add a provider-capability dispatch surface to the deployment CLI or a deployment-owned companion
  command for reviewed capability hooks.
- Wire concrete catalog entries for the recommended AWS/Supabase/Cloudflare/Vercel topology to
  executable hook adapters rather than generic placeholder commands.
- Support hook phases for:
  - preview/diff
  - apply/mutation sequence
  - evidence capture
  - smoke checks
  - rollback
- Require every hook invocation to bind to the concrete capability id, target identity, credential
  source, lock scope, replay semantics, and audit evidence declared in the catalog.
- Redact hook output before it can be persisted as control-plane evidence or shown in operator
  guidance.
- Keep support-mediated prerequisites represented as evidence-gated manual steps, but require the
  same concrete capability id and audit evidence contract.
- Preserve the boundary between deployment control-plane orchestration and Buck2 remote execution or
  Nix remote-builder scheduling.

### 3. External prerequisites

- None for fixture tests.
- Optional provider credentials for live-gated provider preview/smoke checks.

### 4. Tests to be added

- Add fixture tests for provider-capability dispatch by phase and capability id.
- Add negative tests for unknown capability ids, unsupported phases, mismatched target identity,
  missing credential source, missing lock scope, missing audit evidence refs, and unredacted output.
- Add setup/cutover tests proving evidence generated by a hook can satisfy readiness only for the
  matching selected capability.
- Add tests proving generic placeholder commands or static-only declarations cannot satisfy
  protected/shared readiness.
- Add live-gated provider preview/smoke hook tests for selected capabilities, skipped by default.

### 5. Docs to be added or updated

- Update `docs/history/designs/cloud-control-design.md` with the executable hook dispatch contract.
- Update `docs/cloud-control-setup.md` and `docs/cloud-control-cutover.md` with operator workflows
  for running provider-capability hooks and reviewing captured evidence.
- Document which capabilities have automated hooks and which remain evidence-gated prerequisites.

### 5.5. Expected regression scope

- `deployment-only`
- If provider-specific IaC modules are added outside deployment-owned paths, update this plan with
  the expanded regression scope before implementation.

### 6. Acceptance criteria

- Provider-capability hooks can be invoked through a reviewed deployment-owned dispatch path.
- Hook evidence is tied to the concrete capability declaration and cannot satisfy unrelated
  capabilities.
- Placeholder commands, dashboard-only notes, raw IaC state, or static declarations alone cannot
  satisfy protected/shared readiness.
- Live provider checks remain explicitly gated and safe for non-production verification.

### 7. Risks

- Executable provider hooks can expand the trusted control-plane surface.
- Provider CLIs can emit secret-bearing output or unstable provider-specific metadata.

### 8. Mitigations

- Keep default tests fixture-based and require explicit live gates for provider credentials.
- Redact all hook output and persist only declared audit evidence fields.
- Require lock scope and replay semantics before running mutation phases.

### 9. Consequences of not implementing this PR

Provider-capability readiness can be structurally validated but still lacks a reviewed execution path
for producing the evidence it requires.

### 10. Downsides for implementing this PR

The deployment tooling must maintain a provider-capability hook API and fixture adapters for each
recommended topology component.

## PR-19: Closed image digest contract across build and publication

### 1. Intent

Close the OCI image digest contract so image metadata, labels, publication evidence, and generated
host-profile pinning all agree on immutable build and registry digest semantics.

### 2. Scope of changes

- Replace nullable or `unpublished` digest metadata with an explicit digest contract that
  distinguishes build identity from verified registry publication digest.
- Emit image labels and contract artifacts that report the verified digest relationship instead of
  placeholder or null values.
- Wire publication planning and registry inspection evidence back into the image contract/status
  consumed by setup, cutover, and host-profile generation.
- Ensure generated NixOS, Compose, and Podman profiles pin to the verified publication digest when
  publication evidence is present.
- Fail closed when production setup or cutover requests an image digest but only build identity,
  placeholder, null, or unverified publication metadata is available.
- Keep local fixture/dev image flows explicit so default tests can run without publishing to a live
  registry.

### 3. External prerequisites

- None for fixture tests.
- Optional local image build and registry tooling for live-gated digest verification.

### 4. Tests to be added

- Add image contract tests proving digest labels and contract fields are never placeholder, null, or
  `unknown` in production/publication mode.
- Add publication tests proving registry inspection evidence matches the recorded publication digest.
- Add setup and host-profile tests proving generated profiles pin to the verified digest when
  publication evidence is supplied.
- Add negative tests for mismatched build identity, mismatched publication digest, missing registry
  inspection evidence, and production profiles that request digest pinning without verified evidence.
- Add live-gated image build/publish/inspect verification tests, skipped by default.

### 5. Docs to be added or updated

- Update `docs/history/designs/control-plane-containerization.md` with build identity vs registry digest semantics.
- Update `docs/cloud-control-setup.md` and `docs/cloud-control-cutover.md` with digest evidence and
  host-profile pinning requirements.
- Update any runtime or publication docs that describe image contract fields.
...
- Production image contract metadata reports verified digest semantics instead of null, placeholder,
  `unknown`, or `unpublished` digest state.
- Host-profile digest pinning, publication evidence, registry inspection, and image contract metadata
  agree.
- Setup and cutover reject unverified or mismatched digest evidence for production profiles.
- Local fixture/dev flows remain explicit and do not masquerade as verified publication evidence.

### 7. Risks

- Build-time and registry digest values can differ depending on image format and publication path.
- Registry inspection availability can vary across local and hosted registries.

### 8. Mitigations

- Model build identity and registry publication digest as separate fields when they are distinct.
- Keep live registry verification gated and provide fixture evidence for default validation.

### 9. Consequences of not implementing this PR

Operators can still generate or inspect image metadata that does not prove the image digest pinned in
host profiles matches registry publication evidence.

### 10. Downsides for implementing this PR

Image publication and setup flows become stricter and require additional evidence files for
