...
Status: historical stabilization note. The completed setup and validation details here are retained
for traceability; use `docs/README.md` to find current setup, deployment, and validation runbooks.
...
- Old plans do not look like active implementation instructions when they are already complete.
  Status: complete for `docs/history/plans/local-sprinkleref-plan.md` and
  `docs/history/plans/control-plane-selector-plan.md`; both now have historical implementation-plan headers.
...
- Update [AWS Control Plane Setup Guide](../../control-plane-guide.md) to make the setup-plan command
...
- Update [Deployment And Secrets API](../../deployment-secrets-api.md),
  [Deployments Usage](../../deployments-usage.md), and
  [Troubleshooting](../../handbook/troubleshooting.md) where token-resolution wording changes.
...
- Update [AWS Control Plane Setup Guide](../../control-plane-guide.md) and relevant handbook
...
- Update `docs/local-sprinkleref.md` examples to include `controlPlanes` and per-context
  `controlPlane`.
- Replace the stale example domain in `docs/control-plane-selector.md` or make it an explicit
  checked-in shared config example.
- Mark `docs/history/plans/local-sprinkleref-plan.md` and `docs/history/plans/control-plane-selector-plan.md` as completed or
