...
This document is a development plan to implement Phase 4 from `docs/history/build-system/linking-roadmap.md`. I keep the plan as a list of PRs. Each PR includes its own tests and documentation updates. I do not plan any tests-only or docs-only PRs. No functionality should land without tests in the same PR.
...
- Update `build-tools/docs/go-linking.md` to document `link_closure` and `link_closure_overrides` for cgo targets.
- Update `build-tools/docs/cpp-linking.md` to note that Go cgo consumers can opt into following C++ `link_deps`.
