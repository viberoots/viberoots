...
- Update `build-tools/docs/abstractions.md` to call out the overlay as using the shared decoder, so any future drift
  is reviewable.
...
- Update `build-tools/docs/abstractions.md` to list the C++ overlay as a consumer of
  `build-tools/tools/nix/lib/lang-helpers.nix:decodePatchFilename`.
