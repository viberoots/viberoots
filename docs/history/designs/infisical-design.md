...
`DeploymentSecretContractBinding` should continue to carry the backend kind and
logical contract id separately. The logical contract id remains the
backend-neutral SprinkleRef URI, while backend replay/storage identifiers must
not turn that URI into an Infisical key containing `secret://`:
...
synthetic, but it should use the selected backend kind and a scheme-free
selector:
