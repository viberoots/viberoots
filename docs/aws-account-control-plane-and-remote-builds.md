...
coordinates, and use `sprinkleref --update ... --category bootstrap --create-missing` to write the
token to the configured bootstrap backend when needed:
...
The local redirect generated by `sprinkleref --init-local` chooses `category: "bootstrap"` for the
token ref. If SprinkleRef is not ready yet, export `SUPABASE_ACCESS_TOKEN=<token>` in the setup
shell for that run. The env var wins when both the env var and ref are present.
