...
| PR-10  | _pending scope review and commit_        | Post-scope-fix focused validation from the parent workspace: `i` (`.codex-logs/pr10/focused-expanded-parent-i-20260619-1529.log`), `b` (`.codex-logs/pr10/focused-expanded-parent-b-20260619-1529.log`), and `v viberoots//:viberoots_remote_consumer_fixture viberoots//:viberoots_nix_split_flake_fixture viberoots//:lib_workspace_activation viberoots//:dev_nix_build_filtered_flake_cpp_selected_scope` (`.codex-logs/pr10/focused-expanded-parent-v-rerun-20260619.log`; verify log `/Users/kiltyj/Code/viberoots/.viberoots/buck/verify-logs/verify-2026-06-19T19-39-10-003Z-25643-d725326f3ec4e.log`); direct expanded remote-consumer fixture passed after lock-refresh in `.codex-logs/pr10/direct-remote-consumer-after-lock-refresh-20260619.log`; real GitHub SSH flake import smoke passed against scratch ref `refs/heads/codex/pr10-remote-smoke-20260619163947` at `a8d21caafcbff9b0717863ef484d00f7d57ce17f` (`.codex-logs/pr10/direct-remote-consumer-real-scratch-ref-rerun-20260619.log`, ref record `.codex-logs/pr10/remote-smoke-scratch-ref-20260619.txt`); focused target rerun passed in `.codex-logs/pr10/focused-remote-consumer-after-lock-refresh-v-20260619.log` (verify log `/Users/kiltyj/Code/viberoots/.viberoots/buck/verify-logs/verify-2026-06-19T20-15-59-742Z-81148-948243af1f189.log`); scoped PR-local lint/prettier/file-size gates passed in `.codex-logs/pr10/scoped-eslint-after-lock-refresh-20260619.log`, `.codex-logs/pr10/scoped-prettier-after-lock-refresh-20260619.log`, and `.codex-logs/pr10/file-size-after-lock-refresh-20260619.log` | Final validation and plan assessment deferred until the required `$prs` scope review passes; because PR-10 touched shared filtered-flake build snapshot logic, final validation must include full `i && b && ALL_TESTS=1 v` | Remote fixture, activation, status, split-flake source metadata, selected C++ snapshot coverage, two-consumer ownership boundary checks, root plus `projects/` bare-command smoke coverage, and opt-in published-ref smoke coverage prove the remote source-store path and hidden workspace Buck graph contract before final checkpoint validation |

PR-10 final checkpoint evidence (2026-06-20): focused recovery validation passed with
`v viberoots//:viberoots_remote_consumer_fixture viberoots//:viberoots_nix_split_flake_fixture viberoots//:lib_workspace_activation viberoots//:dev_nix_build_filtered_flake_cpp_selected_scope`
in `/tmp/viberoots-codex-logs/pr10/focused-v-bootstrap-devpath-20260619-212649.log`.
The first full final `i && b && ALL_TESTS=1 v` run reached serial `Pass 18 Fail 0`,
main `Pass 1450 Fail 1`, and resource-limited `Pass 213 Fail 0` in
`/tmp/viberoots-codex-logs/pr10/final-i-b-all-tests-v-clean-20260620-020204.log`; the lone
top-level failure was `viberoots//:python_python_pyext_imported_by_pylib_runtime_import` from an
invalid local Nix store path, and the targeted rerun passed in
`/tmp/viberoots-codex-logs/pr10/focused-python-pyext-runtime-20260620-035842.log`. The repeat clean
full final run passed in
`/tmp/viberoots-codex-logs/pr10/final-i-b-all-tests-v-repeat-20260620-035958.log`: serial
`Pass 18 Fail 0`, main `Pass 1451 Fail 0`, resource-limited `Pass 213 Fail 0`, `exit_code=0`,
elapsed `6106s`. Scope review passed before the full final run.
