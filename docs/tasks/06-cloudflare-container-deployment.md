...
- A front-door gate (`cloudflare-containers-front-door.ts`) — hard-blocks `preview`, `publish-only`, `rollback`, and `provision-only`. Blocks protected/shared mutation with a hard error when `requireServiceForProtectedShared` is true or a `--control-plane-url` flag is present. Only `local_only` deploys reach `submitCloudflareContainersDeploy` today.
...
2. **Admission path for protected/shared deployments** — connect the front-door to the control-plane service (the containerized control plane from #4). The front-door already contains the scaffold (`requireServiceForProtectedShared`, `--control-plane-url` guard) but the actual service call, evidence binding, and snapshot freeze are absent.
