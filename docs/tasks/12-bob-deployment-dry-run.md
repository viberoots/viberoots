...
2. Run the remote plan to confirm destination, artifact identity, and service client block:
   `deploy --deployment <label> --profile the current personal server --plan`.
3. Run the full deploy:
   `deploy --deployment <label> --profile the current personal server --artifact-dir ./dist`
...
4. If the run returns `pending_approval`, check status and approve:
   `deploy --deployment <label> --profile the current personal server --status --text --deploy-run-id $RUN_ID`
   `deploy --deployment <label> --profile the current personal server --approve --deploy-run-id $RUN_ID --approval-id <ref>`
...
the current personal server is serving the deployment control plane endpoint directly, the session is at risk from any host
...
**Live run on the current personal server vs. a disposable fixture environment.** Running against the current personal server
...
smoke failures on the current personal server do not affect live traffic.
