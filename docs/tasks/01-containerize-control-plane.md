...
**Estimated effort:** XL
**Date:** 2026-05-25
**Summary:** Transform the deployment control plane from a self-hosted service into a cloud-portable, horizontally scalable containerized runtime, then migrate it off the current personal server to an external cloud host.

## What

Implement `docs/control-plane-plan.md` end to end: ten sequenced PRs that transform the deployment
...
sequence in `docs/cloud-control-design.md`: prove external persistence (Supabase Postgres +
Supabase Storage or R2), make the current personal server cloud-shaped by pointing it at external DB and artifact store,
introduce cloud host running the same image by digest, migrate auth to an abstracted provider
(Supabase Auth or WorkOS), cut traffic to the cloud host, retire the current personal server's authority.

The current the current personal server serves `the deployment control plane endpoint` and `the auth endpoint` through nginx
...
- the Pleomino Infisical cutover (PR-12 in `docs/infisical-plan.md` consumes the PR-1
...
stage-state paths that are already in production use on the current personal server. A subtle regression in fencing-token
...
**Local Postgres migration.** Moving the current personal server's control-plane database to external Postgres requires
...
image and module. The six-phase cloud migration in `docs/cloud-control-design.md` is the operational
plan for actually cutting traffic. Phases 1–2 (prove external persistence, make the current personal server cloud-shaped)
can begin as soon as PR-3 and PR-4 land and the current personal server's workers are pointed at an external Postgres and
artifact store. Phases 3–6 require the reviewed image from PR-7 and a chosen cloud host substrate.

**The plan is the authoritative sequence.** Each PR must update `docs/control-plane-plan.md` if
