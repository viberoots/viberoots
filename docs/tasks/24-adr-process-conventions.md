...
- Do not backfill missing ADRs for decisions that predate the viberoots rename or for decisions
  already captured in design docs under `docs/`. The existing design documents (`docs/deployments-design.md`,
  `docs/infisical-design.md`, etc.) are not ADRs and do not need to become them. The ADR corpus
