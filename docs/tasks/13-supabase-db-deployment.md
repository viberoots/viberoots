...
Postgres." Phase 2 is "Point the current personal server control-plane service and workers at external Postgres." Without
a provisioned and tested Supabase Postgres target, Phase 1 and 2 cannot close, the current personal server cannot be made
cloud-shaped, and task #4 (Containerize Control Plane) cannot reach its cut-over gates.
