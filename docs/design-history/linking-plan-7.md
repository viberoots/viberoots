...
- Update `docs/design-history/python-extension-design.md` to explicitly state that `uv.lock` is a required input for `T.pyExt` even when `build_py_deps` is empty.
