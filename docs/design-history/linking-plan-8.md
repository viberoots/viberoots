...
- Update `build-tools/docs/cpp-linking.md` to state that `link_closure_overrides` is consumed by `nix_cpp_binary`, `nix_cpp_library`, and `nix_cpp_node_addon` and must reference only `link_deps`.
...
- Update `build-tools/docs/cpp-linking.md` to state that `header_deps` are applied to C++ libraries during their own compilation.
...
- Update `build-tools/docs/cpp-linking.md` to include a shared lib addon example and note any runtime constraints.
