...
- Update `build-tools/docs/cpp-linking.md` to state that `header_deps` never add link inputs, even if the dep is a C++ library target.
- Update `build-tools/docs/cpp-linking.md` to clarify that overlap is allowed, but link inputs come only from `link_deps`.
...
This PR resolves the remaining inconsistency in `build-tools/docs/cpp-linking.md` about overlap between `link_deps` and `header_deps`, and adds a targeted test that reflects the clarified behavior.
...
- Update `build-tools/docs/cpp-linking.md` to remove the disjoint requirement and describe overlap as allowed.
- Update `build-tools/docs/cpp-linking.md` to restate that the deterministic union rule applies to all three lists.
