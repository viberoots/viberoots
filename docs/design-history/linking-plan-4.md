...
This document is a development plan to implement **Phase 3** from `build-tools/docs/linking-roadmap.md`.
...
- Extend the Python templates/uv adapter wiring to expose a stable “wheelhouse env” for build-time use:
  - reuse the existing wheelhouse derivation contract (as described in `docs/design-history/python-extension-design.md`)
