...
This plan turns the design in `docs/pnpm/pnpm-exporter-adapter.md` into a sequence of focused PRs. We keep macros as the single source of truth (SST) for Node importer‑scoped labels and patch invalidation, add a validate‑only exporter adapter, emit a deterministic sidecar index, and introduce a composite graph API for all tooling.
...
- Cross‑link from `build-tools/docs/build-system-design.md` and `docs/pnpm/pnpm-exporter-adapter.md` to the composite API and sidecar description.
