...
- Intentional pnpm, Go, or Python/uv dependency upgrade: run `u --upgrade`, then `i && b && v`.
...
- `i` and post-clone are read-only for tracked metadata. Stale dependency metadata fails with
  `repair: run u`; intentional dependency version movement uses `u --upgrade`; source-pin or
  generated source-configuration drift uses `viberoots update`.
- `viberoots update` moves only viberoots pins and performs the deterministic reconciliation required
  by the new tooling revision. It never upgrades project dependencies.
...
- Build/test:
  ...
  - Policy gate (inventory + exceptions): `node viberoots/build-tools/tools/dev/nix-gaps-inventory-check.ts --starlark-api docs/handbook/starlark-api.md --nix-gaps docs/handbook/nix-gaps.md --exceptions docs/handbook/nix-gaps-exceptions.json --command-site-policy docs/handbook/nix-command-site-policy.json`
...
- **Stage mutable temp flakes before Nix archives them**. A native reconciliation fixture evaluated an unversioned temp flake after its artifact environment had written `buck-out/tmp/artifact-environment/xdg-cache/nix/fetcher-cache-v4.sqlite`; identical warm runs therefore registered different source identities. Initialize fixture flakes as Git worktrees, ignore `.nix-gcroots`, `.viberoots`, `buck-out`, and result links, and stage only declared source plus the generated lock before evaluation. The corrected cold archive contained none of those generated roots, and its immediate 226-second repeat added zero Nix paths with only 94 KiB of Nix-volume movement.
- **Register artifact evaluation bundles once by content identity**. Selected and full artifact evaluators consume the store-backed `source/` inside `viberoots.evaluation-bundle.v1`, never the staging tree or live worktree. Keep selection, classification, dependency identities, and the sorted file manifest deterministic. CoW and full-copy staging must register the same NAR path, and warm repeats must not create a second bundle identity.
- **Treat bundle construction roots as owned transient state**. Reject external symlinks and unsupported entries before registration, bound the owned Nix command, await child shutdown, and remove the construction root after success, failure, timeout, interruption, or hard owner death. Hard-death cleanup must bind the owner PID and nonce to the registration process group, stop that group before cleanup, and remove only a marker-matching root. Do not use GC, raw store mutation, live-path fallback, or broad snapshot expansion as bundle cleanup.
- **Keep exact-store materialization explicit and durably rooted**. Only `u` or an explicit bootstrap reconciliation path may build a missing exact pnpm store. A successful materialization must atomically establish the importer-scoped ignored `.nix-gcroots/pnpm-store.*` root before returning. `i`, post-clone, node-module linking, and devshell entry remain probe-only: they may validate or reassert a root for an already-realized committed path, but a missing or invalid path fails closed with `repair: run u` and never starts a Nix build. Only explicit reconciliation may replace a stale owned exact-store root.
...
- **Keep the universal pnpm FOD libc-deterministic across Darwin and Linux**. pnpm 11.5.3 documents `supportedArchitectures.libc`, but its platform check skips libc when Darwin reports the current libc as unknown. A pinned real `sharp` fixture with exact Linux arm64/x64 glibc markers and no `--force` still retained all four musl packages in 3 seconds, using 68,312 KiB. The root store's six unavoidable musl records account for 505,788 KiB of unique content, primarily the two Claude binaries. Universal Linux fetch markers therefore retain both glibc and musl explicitly so Darwin and Linux builders produce one FOD hash; per-host Nix `node_modules` markers remain glibc-only. Musl is not a supported Nix platform. Do not post-prune pnpm's msgpack index or content-addressed files.
- **Canonicalize SQLite writer-version bytes after rebuilding pnpm's index**. Root-flake and consumer-flake reconciliation produced equal 2,691,544 KiB stores with the same 12,511 files and 517 logical index rows, but bytes 96..99 of `index.db` recorded different Nixpkgs SQLite writer versions. Zero that non-semantic header field after `ANALYZE; VACUUM`; the rewritten database must still pass `PRAGMA integrity_check`. The converged parent `u` run completed both importer mismatch/verification pairs in 202.89 seconds with a 3,349,748 KiB peak under the 8 GiB guard. Do not accept hash ping-pong between root and nested views of one lockfile.
- **Reject wrong pnpm FOD hashes before daemon registration**. After `postBuild`, the fixed-store builder hashes its completed `$out` with the pinned Nix tool. On mismatch it removes only that builder-owned output, verifies the path is absent, and only then emits the versioned specified/got marker used for the retry. A real native fixture completed two mismatch/cleanup passes plus final offline consumption in 52.87 seconds with 8,352 KiB net disk growth. Keep mismatch cleanup ahead of metadata mutation; do not replace it with GC, broad store-family deletion, or raw store mutation.
...
- **Wait for guarded child shutdown before removing its temp root**. A disk or time guard must terminate the owned process group, await the child `close` event, and then report the original guard error before fixture cleanup begins. Rejecting immediately lets a still-exiting Nix process race recursive removal of its XDG cache; the resulting `ENOTEMPTY` replaces the disk evidence that triggered termination.
...
- **Do not retain exact node modules from a shell or command closure**. An eager
  `viberootsNodeModules` edge from the development shell and packaged `viberoots` command retained a
  5,278,293,400-byte output for every changed derivation. After removing that edge, a cold exact
  development-shell build realized only the command wrapper and shell in 2 seconds, added 7,660 KiB,
  and kept the named large-output count at 12. The immediate warm repeat completed in 0 seconds with
  zero new store paths. Keep explicit node-module attributes available for `u` and `i`, but make
  shell entry probe/relink only and keep `NODE_PATH` out of the shell and packaged command.
- **Use measured pnpm checkpoints when changing reconciliation**. Focused evidence for the current
  root lock recorded: a small native fixture at 26.83s and under 500 MiB; root reconciliation at
  106.13s with a 5.19 GiB clean-baseline peak; an identical forced rebuild at 95.71s with a 9.66 GiB
  peak and no persistent new large output; and a warm realized-output probe at 3.29s with zero new
  paths. Exact-store realization is explicit dependency work, not a development-shell side effect.
- **Distinguish cold exact-store fixture work from retained growth with a warm repeat**. One cold
  verify window registered 7,936,778,688 NAR bytes, including 6,651,016,872 bytes across nine pnpm
  stores and thirteen importer-specific node-module outputs, with no invalid store residue. Equal
  lock hashes do not make node-module outputs interchangeable because pnpm launchers and workspace
  metadata retain importer/output identity. An actual canonical warm rerun of a large SSR fixture
  registered the derivations but zero pnpm-store or node-module outputs and no path at or above 100
  MB. Use the Nix `ValidPaths` registration window plus owned temp/Buck roots for attribution;
  filesystem-wide `df` deltas alone can include unrelated APFS volume movement.
...
- **Treat a lock-matched immutable generated-workspace input as structurally fresh**. If the generated flake URL and the lock's `locked` and `original` paths all name the same accessible `/nix/store/<32>-source`, and the locked node has a valid NAR hash, lock repair must not run `nix flake metadata` on the enclosing live workspace. Keep stale, missing-hash, inaccessible, and non-store inputs on the repair path. Also exclude runtime-only `.viberoots/workspace/host-path` from Nix snapshots; Codex and Claude wrappers read it after shell entry, but no flake or Nix expression imports it. In focused evidence, the pre-fix metadata call created a new live-workspace source containing Buck, install-cache, smoke, host-path, and temp-root state on every temp fixture. The fixed cold run added only three clean content-addressed sources and no FOD paths; its identical 34-second repeat added zero source or FOD paths, and sampled argv contained no live-workspace metadata call or viberoots override.
- **Never prefetch a live viberoots worktree as a raw path input**. A temp-repo helper passed the live checkout directly to `nix flake prefetch`; Git path capture retained ignored `.viberoots/workspace` state and registered a valid 5,263,393,960-byte source whose generated workspace alone occupied about 5.0 GiB. A later full run reconfirmed the same defect in a split-flake fixture, which registered a 5,263,456,880-byte raw source. Temp fixtures now execute tools from the live checkout but lock a separately filtered `viberoots-flake-input`. The focused cold fixture produced one 18,497,592-byte source with generated workspace Buck/log/install state absent; the identical warm run produced zero new source or node-module paths.
- **Reference filtered viberoots inputs by their immutable store identity**. Do not copy the same filtered `viberoots-flake-input` into every consumer snapshot: one full run registered 44 consumer sources containing 1,128,292,896 bytes of nested viberoots inputs, including 32 byte-identical copies. Materialize the already filtered input once, require its `/nix/store/*-source` path, and write that absolute path into the consumer flake and lock before Nix sees the outer snapshot. The focused real fixture registered one 18,497,592-byte filtered source on its cold run; an independent identical repeat reused the same path and registered zero new sources. The consolidated `runInTemp` helper likewise registered one 18,498,760-byte source across two independent scratch fixtures, and a second complete test-process repeat reused that source with zero new source paths.
- **Never override seed or fixture flake inputs with a live checkout**. Verifier seed builds must trust the generated workspace lock's relative `./viberoots-flake-input` identity; do not replace it with `path:$PWD/viberoots` from an environment fallback or test command. A live override registered a 5,263,860,296-byte source whose `.viberoots/workspace` occupied 5,147,388 KiB. After removing both override paths, the focused cold production run's largest new objects were a 38,190,096-byte test seed and roughly 25.8 MB filtered sources; an independent 24.9-second warm run registered zero new Nix paths. Attribute this class of growth by querying the Nix `ValidPaths` registration window (`registrationTime`, `narSize`, and `path`) and then inspect the largest new source before cleanup.
- **Do not evaluate a live checkout as a path flake, even for metadata or cold-start tool discovery**. A full validation run registered a 5,263,861,872-byte `-source` when a metadata test evaluated `path:<live-viberoots>`; the captured source contained 5,147,388 KiB of ignored generated workspace state. Equivalent fixture overrides and the cold `toolchain-paths.json` generation path had the same latent exposure. Metadata tests must use the filtered-flake authority, generated-workspace fixtures must trust their immutable lock, and bootstrap/install tool discovery must use `.viberoots/workspace/flake.nix` or fail before invoking Nix. The focused 10-target regression registered no path at or above 100 MB; its largest object was a 38,188,792-byte test seed and its filtered sources remained roughly 18.7-25.9 MB.
...
- **Keep configured Nix caches from becoming a hard gate**: `i`, `b`, `v`, and Buck Nix actions dynamically perform one bounded `nix-cache-info` request for each configured HTTP(S) substituter and remove unreachable optional caches from the current command while keeping required caches configured when `VBR_NIX_CACHE_POLICY=auto` (the default). In auto mode, HTTP/authentication failures such as 401 degrade only optional substituters; required substituters and strict mode fail closed. Because effective Nix output flattens `extra-substituters`, canonical ingress reconstructs role provenance from the authoritative configuration sources and admits it only when the combined role members match the effective configuration exactly. Ambient applied/reviewed environment markers are result metadata, not reusable authority. Only canonical command ingress may reuse a review after its FD proof binds the exact config bytes, policy, and required/optional lists into an in-memory capability; Buck action re-review verifies the same binding. Nested test environments do not receive applied/reviewed markers and perform their own review. Nix fallback remains enabled, so ordinary local validation should continue without the cache. Authenticated probes use the reviewed Nix `netrc-file` without logging credential contents. Use `VBR_NIX_CACHE_POLICY=strict` only when cache availability is itself being tested, and `VBR_NIX_CACHE_POLICY=off` only when you need to skip the dynamic probe.
  Cache URLs must not embed credentials in userinfo, query parameters, or fragments; configure
  authentication through `netrc-file`. Credential-bearing URLs fail closed before review or probe.
  Percent-encoded query or fragment keys are rejected as ambiguous, and every fragment parameter
  is checked. Buck action shells discard ambient review markers and re-review the exact active
  configuration after copying any readable `netrc-file` into action-private state.
  Changes to cache health, artifact Nix policy, command ingress, generated toolchain policy, or Buck
  Nix action shells must include `dev_nix_cache_health`, `dev_nix_cache_health_command_scope`,
  `dev_nix_cache_health_shell_handoff_integration`, `dev_artifact_network_policy`,
  `dev_artifact_build_policy`, and `lang_nix_action_runner_cmd_snippets_cquery` in the affected
  validation union.
  When a child sanitizes `NIX_CONFIG`, validate the exact forwarded bytes and do not forward
  applied/reviewed markers; the child must perform its own review. Generated
  shell probes must classify curl exit 35 (TLS connection failure) as a transport outage so auto
  policy can remove the unavailable cache instead of failing the build.
...
- **Nest cold-fixture timeouts inside the production envelope**: a cold integration fixture that exercises a production operation with a 600s budget must not retain a shorter leaf command timeout that kills healthy work first. Give managed leaf commands the production budget, keep the enclosing test timeout larger than the leaf budget plus setup and shutdown margin, and prove timeout cleanup reaps the whole owned process group.
...
- **Strip orchestration preloads at external Node-worker boundaries**: test harnesses may add `--import zx-init.mjs` to `NODE_OPTIONS`, but pnpm and other external worker-based Node tools must not inherit that preload. Use the canonical environment sanitizer at the subprocess boundary, preserve unrelated Node options, and test the child-visible environment; retries, serialization, and worker-count limits do not fix preload leakage.
- **Do not pass Node-managed file descriptors to external worker-based tools on macOS**: a parent that opened capture files with `fs.promises.open()` and passed `FileHandle.fd` values as child stdio caused repeatable pnpm `EXC_GUARD` terminations when a worker closed inherited descriptor 19. Keep file-backed capture, but have the canonical shell open the output paths through redirection and `exec` the tool so PID, process-group, timeout, and evidence ownership remain unchanged. The focused worker-child regression and the previously failing SSR/pnpm reconciliation target passed without a new crash report; retries, serialization, and worker limits do not repair descriptor ownership.
...
- **Share proven pnpm-store hashes by exact derivation authority**: when many temp repos reconcile the same importer, key the shared lock and verified hash result by the evaluated pnpm-store derivation identity plus lock hash, scope that lock to the canonical user-global hash-authority root rather than each temp repo, and publish a successful repair under both the pre-repair and final derivation identities. Whole consumer flake locks and hand-maintained builder-file fingerprints fragment equivalent work or omit real inputs; a global cache with repo-scoped locks still permits concurrent duplicate rebuilds, while repo-local markers alone let every temp repo repeat the same fixed-output rebuild.
- **Preserve real nested Git topology in verify seeds**: a copied viberoots source must have one deterministic nested commit and the seed parent must record its matching `160000` gitlink. Flattening `viberoots/**` into the consumer index changes importer/source authority, so `u` can mutate the copied hash map while Nix evaluates a different immutable input. Construct this topology once in the shared seed (and advance the seed protocol marker when it changes), then commit staged nested overlays and advance the parent gitlink rather than re-indexing the source in every temp repo.
- **Pack nested seed repositories before publication**: disabling automatic Git maintenance while
  creating the deterministic nested commit can leave thousands of loose objects that every
  `runInTemp` clone must traverse. Repack the nested repository with one thread before advancing
  the parent gitlink, then require a clean full `git fsck`, zero loose objects, at least one pack,
  no object alternates, and an exact parent gitlink before publishing the prepared marker. Keep
  copied seeds self-contained; do not use `git gc`, alternates, or shared object stores.
...
- **Serialize only proven heavy fanout inside the resource lane**: keep the reviewed `verify:heavy-fanout` taxonomy in the existing two-thread `verify:resource-limited` pass and acquire the shared `viberoots_heavy_fanout` Buck local resource. Do not create another pass or spend a worker permit while waiting for the resource; one heavy test may overlap one ordinary test, but two heavy tests must never overlap.
- **Report high-load sampler health, not just samples**: resource summaries must distinguish zero high-load attempts from sampler unavailability, timeout, or error, and retain a bounded last successful top-process sample. A zero sample count without attempt/outcome counters is not evidence that the host had no high-load process.
...
- **Require absolute XDG cache roots across temp-workspace boundaries**: a relative inherited
  `XDG_CACHE_HOME` is resolved against each command's cwd. Under nested consumer fixtures this can
  create and commit mutable Nix cache state inside the synthetic source, producing a new source
  identity on every warm run. Temp orchestration must replace a relative value with its canonical
  absolute shared cache root before invoking commands. The focused remote-consumer regression
  completed in 224 seconds with zero new Nix paths on the unchanged warm rerun and 3,756 KiB of
  contextual `/nix` volume movement.
...
- **Launch temp-workspace command fixtures from the temp workspace**: setting `WORKSPACE_ROOT` does not authorize a live-checkout cwd to resolve an unrelated temp tree. Root discovery intentionally rejects that mismatch and can silently select the active live verify instead, turning short status/watch tests into full timeout stalls. Keep the executable path anchored to the live tool source when needed, but set the child `cwd` to the temp workspace whose state it is meant to observe.
...
- **Keep orphan cleanup aligned with pass-specific verify isolation names**: verify may create roots such as `v-<owner>-<run>-resource-limited.noindex`, `v-<owner>-<run>-shared.noindex`, and `verify-nested-<owner>-<hash>.noindex`. Cleanup must parse the encoded owner PID, protect only live owners, reap dead-owner pass roots after interrupted runs, and remove the generated `buck-out` roots while preserving `.metadata_never_index`.
...
- **When scoping env for graph/setup, preserve selectors only until bundle registration**: pass required keys such as `BUCK_TARGET`, `BUCK_GRAPH_JSON`, and workspace roots through pre-bundle graph/setup subprocesses. Capture their reviewed values in the immutable evaluation bundle, then strip them from downstream `nix build` environments.
- **Filtered flake helper builds must register selector-complete evaluation bundles**: capture the graph, target, C++ slicing mode, wasm backend, dependency metadata, and reviewed development overrides in the immutable bundle before Nix evaluation. Normal helpers must strip the former selector environment and must not use `--impure`; override source trees must be copied into the bundle and rewritten to immutable bundle paths, or fail closed before evaluation.
...
- **Stage updated pnpm-store hashes in temp repos**: when a consumer test updates `projects/config/node-modules.hashes.json`, `git add` it before any Nix builds so the flake snapshot sees the new hash instead of the placeholder. Standalone viberoots tooling hashes stay in `build-tools/tools/nix/node-modules.hashes.json`. If a test generates a new `pnpm-lock.yaml`, always regenerate its hash even if an older entry exists in the map.
...
- **Pair resource-limited scope with heavy-fanout classification for nested workspace tests**: `verify:resource-limited` lowers Buck target concurrency, but it does not prevent two expensive temp-workspace/Nix/Buck/Node tests from running together. Tests that scaffold consumer workspaces, refresh lockfiles or hashes, build Tauri/Rust/WASM artifacts, run nested `b`/`v`, or launch dev servers under resource-limited scope should also be listed in `heavy_fanout_taxonomy.bzl` and covered by the heavy-fanout contract so the shared local-resource permit serializes the real fan-out.
- **Give concurrent verify passes distinct Buck isolations and stage broad groups resource-first**: after serial `verify:isolated` work, each pass needs a distinct top-level Buck isolation. For broad groups with at least 50 resource-limited and 500 shared targets, finish the two-thread resource-limited lane before starting shared work; smaller focused groups may run their lanes concurrently. A timed overlap is an explicit diagnostic override, not the default.
...
- **Keep ordinary builds warning-only under disk pressure**: `b` must not run Nix store optimisation or GC by default. Low free space should report the pressure and direct operators to `viberoots gc --dry-run` plus an explicit maintenance command outside build/verify. `VBR_OPTIMISE_MODE=always` or `VBR_GC_MODE=auto` are explicit maintenance opt-ins and still honor the live verify lock.
...
- **Audit runtime references before timing cache publication**: a small artifact can still have a multi-gigabyte closure. Rust WASM static archives retained Rust source paths from the toolchain even when their payload was only about 9 MB, producing roughly 2.70 GB runtime closures and dominating `nix copy`. Keep manifests and tool identities in a separate provenance output, remove only proven non-runtime toolchain/source references from archives, assert direct references and closure bounds for every runtime kind, and link/execute the rewritten static and WASI consumers before accepting the timing result.
