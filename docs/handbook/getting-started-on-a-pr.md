...
- **Abort verify if GC starts mid-run**: a GC process that starts after preflight can invalidate in-flight Nix paths and produce widespread false negatives (`path ... .drv is not valid`, lockfile generation failures, missing/stale glue diagnostics). Treat this as a systemic environment fault, stop verify immediately, stop GC, and rerun the same failing set before any full-suite run.
