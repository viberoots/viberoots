...
- **Keep configured Nix caches from becoming a hard gate**: `i`, `b`, `v`, and Buck Nix actions dynamically perform one bounded `nix-cache-info` request for each configured HTTP(S) substituter and remove unreachable caches from the current top-level command when `VBR_NIX_CACHE_POLICY=auto` (the default). A new command re-probes so a long-lived shell cannot preserve stale network state; nested children share the command-local result. Nix fallback remains enabled, so ordinary local validation should continue without the cache. Authenticated probes use the reviewed Nix `netrc-file` without logging credential contents. Use `VBR_NIX_CACHE_POLICY=strict` only when cache availability is itself being tested, and `VBR_NIX_CACHE_POLICY=off` only when you need to skip the dynamic probe.
