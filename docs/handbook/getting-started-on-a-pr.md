...
- **Keep ordinary install materialization read-only with respect to pnpm hash metadata**. `i`, post-clone, and node-module linking should materialize from committed hash metadata and existing fixed `pnpm-store.<importer>` outputs. They must not implicitly recompute hash JSON, import exact pnpm stores, or treat a missing exact-store prewarm as stale install state. Exact-store preparation belongs on the explicit update/build path that is already proving a store, not on the everyday install fast path.
- **Normalize exact pnpm stores before importing them into `/nix/store`**. Raw pnpm store directories can contain path-local and run-local state such as `v*/projects` symlinks into temp/worktree paths, sqlite `index.db` timestamp blobs, `.metadata_never_index`, mtimes, and permission differences. Importing that raw state creates distinct multi-GB `/nix/store/*-pnpm-exact-store-*` outputs for the same importer. Remove or canonicalize that state before `nix store add-path`, and add a regression test that two stores with different temp paths/timestamps hash identically after normalization.
