...
- **Keep ordinary materialization on committed fixed outputs**. `i` and post-clone may realize the literal fixed-output path derived from valid committed lock/hash metadata without gaining reconciliation or hash-discovery authority. Node-module linking and devshell entry remain probe-only. Stale or invalid committed metadata fails closed with `repair: run u`; a merely absent valid output is materialized read-only.
