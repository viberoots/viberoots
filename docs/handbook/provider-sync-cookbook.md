...
- **Go nixpkgs providers (CGO)**: Go macros do not inject direct provider deps for `nixpkg_deps` (alias: `nix_cgo_deps`). Instead, they attach `nixpkg:<attr>` labels and rely on `MODULE_PROVIDERS` from `third_party/providers/auto_map.bzl` to map targets to providers (format: `//third_party/providers:nix_<normalized_attr>`; example: `pkgs.openssl` → `nix_pkgs_openssl`). Do not handcraft names.
