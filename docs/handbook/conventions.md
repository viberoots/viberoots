...
- Generated glue (not committed)
  - `.viberoots/workspace/buck/graph.json`, `.viberoots/workspace/providers/TARGETS*.auto`, `.viberoots/workspace/providers/auto_map.bzl` are generated by Node scripts and not committed.
...
- Graph consumption (Composite Graph API)
  ...
  - Library: `build-tools/tools/lib/graph-view.ts`; CLI: `node viberoots/build-tools/tools/buck/graph-view.ts`.
  - Sidecars: `.viberoots/workspace/providers/provider_index.json` and `.viberoots/workspace/buck/node-lock-index.json`.
  - Schema/version: both `.viberoots/workspace/buck/graph.json` and `.viberoots/workspace/buck/node-lock-index.json` include `$schema` and `version`; the exporter prints a banner pointing to the Composite Graph API on success.
