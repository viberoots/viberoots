...
- Naming: `//third_party/providers:nix_pkgs_<attr>` where `<attr>` is the nixpkgs attribute path with dots replaced by underscores (e.g., `pkgs.openssl` → `nix_pkgs_openssl`).
