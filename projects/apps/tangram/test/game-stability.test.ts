...
describe("game stability", () => {
  it("keeps board state valid across repeated drag+transform cycles", () => {
    ...
    for (let index = 0; index < 40; index += 1) {
      state = tangramGameReducer(state, { type: "piece/select", pieceId: "black-1-1-1-1" });
      state = tangramGameReducer(state, {
        type: "piece/preview",
        pieceId: "black-1-1-1-1",
        position: { x: index % 10, y: (index * 3) % 15 },
      });
      state = tangramGameReducer(state, { type: "piece/commit", pieceId: "black-1-1-1-1" });
      state = tangramGameReducer(state, {
        type: "piece/rotate",
        pieceId: "black-1-1-1-1",
