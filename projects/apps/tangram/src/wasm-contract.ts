...
export async function readWasmModuleBytes(moduleKey: string): Promise<Uint8Array> {
  ...
  const sourcePath = `/${entry.sourcePath.replace(/^\.\//, "")}`;
  const sourceWithBust = import.meta.env.DEV ? `${sourcePath}?t=${String(Date.now())}` : sourcePath;
  const wasmUrl = new URL(sourceWithBust, window.location.href).toString();
  const res = await fetch(wasmUrl, import.meta.env.DEV ? { cache: "no-store" } : undefined);
  if (!res.ok) {
