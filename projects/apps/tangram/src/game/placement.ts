...
export function inBounds(boardSize: BoardSize, cells: readonly Cell[]): boolean {
  return cells.every((cell) => {
    return cell.x >= 0 && cell.y >= 0 && cell.x < boardSize.columns && cell.y < boardSize.rows;
  });
}

export function noOverlap(occupiedSet: ReadonlySet<string>, cells: readonly Cell[]): boolean {
