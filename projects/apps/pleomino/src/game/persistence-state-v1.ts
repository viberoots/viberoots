...
export function parseTransform(value: unknown): PieceTransform | null {
  const objectValue = asObject(value);
  ...
  return { rotation, flipped: objectValue.flipped };
}
...
export function parsePlacedPieces(
  ...
  for (const item of payloadPieces) {
    ...
    placedPieces.push({ instanceId, pieceId, transform, position, isPlaced: true });
  }
  return placedPieces;
}

export function deriveNextPlacedInstanceId(
...
export function sanitizeTransformByPieceId(
  payload: Record<string, unknown>,
  baseline: GameState,
...
export function serializePersistedGameState(state: GameState): string {
  return JSON.stringify({
    version: 1,
    board: { placedPieces: state.board.placedPieces },
  ...
  } satisfies PersistedGameStateV1);
}
