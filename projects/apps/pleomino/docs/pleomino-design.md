...
- PR-16 solved-board solve-button policy is implemented in `projects/apps/pleomino`:
  - solve action policy:
    - regenerate immediately when the board is complete and the current solved layout was fully
      generated by the randomizer with no subsequent user edits
    - no-op when the board is complete and any user edit has been applied, until pieces are removed
      and/or the board is reset
  - policy/state wiring:
    - `src/ui/game-screen-solve.ts` (auto-generated solved-board tracking + solve-button gating)
  - tests:
    - `test/game-screen-solve-policy.test.tsx`
