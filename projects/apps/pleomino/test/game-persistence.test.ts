...
describe("game persistence", () => {
  ...
  it("exports a stable hash key", () => {
    expect(PLEOMINO_URL_STATE_HASH_KEY).toBe("s");
  });
  ...
  it("round-trips state through URL hash persistence", () => {
    ...
    const encoded = new URLSearchParams(hash).get(PLEOMINO_URL_STATE_HASH_KEY);
  ...
  it("clears persisted state hash", () => {
    let url = "/games/pleomino#s=abc";
    const history = {
      replaceState(_state: unknown, _title: string, nextUrl: string) {
        url = nextUrl;
      },
    };
    clearPersistedGameStateFromHash(history, { pathname: "/games/pleomino", search: "" });
    expect(url).toBe("/games/pleomino");
  });
});
