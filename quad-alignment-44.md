...
- Update `abstractions.md` to list a single canonical provider-edge helper for all wiring models.
...
- Update the provider-edge section in `abstractions.md` to point to the canonical helper and
...
- Update `build-system-design.md` to state that planners must not re-implement `kindOf`.
...
- Update `build-system-design.md` and `abstractions.md` to reference the shared planner helper
...
- Update `abstractions.md` to reference the Python patch-map helper and its defaults.
...
- Update `abstractions.md` or exporter doc pages to state that adapters must use the registry.
