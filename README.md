...
| `VBR_TRUST_NIX_USER=0\|1\|prompt` | `prompt`          | Adds the current user to Nix trusted users when confirmed, avoiding restricted-setting warnings from viberoots' generated Nix config. Equivalent CLI flags: `--trust-nix-user`, `--no-trust-nix-user`.     |
...
| `VBR_URL=<flake-url>`             | GitHub Git URL    | Overrides the flake URL consumed by the workspace. The default uses anonymous Git transport to avoid GitHub API rate limits; explicit `github:`, `path:`, `git+ssh:`, and pinned URLs are still supported. |
