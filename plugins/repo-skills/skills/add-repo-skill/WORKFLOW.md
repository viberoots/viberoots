...
- The plugin manifests at `plugins/repo-skills/.codex-plugin/plugin.json` (Codex) and `plugins/repo-skills/.claude-plugin/plugin.json` (Claude Code) should both expose `./skills/`.
- Do not create or rely on user-global plugin or skill caches (such as `~/.codex/skills` or `~/.claude/plugins/`) for these repo-local skills.

When the user wants a new skill, implement the skill files, keep any persistent state clone-local, and tell the user the minimum install, uninstall, and restart steps needed for the agent client to discover the new skill.
...
1. Confirm `plugins/repo-skills/.codex-plugin/plugin.json` (Codex) and `plugins/repo-skills/.claude-plugin/plugin.json` (Claude Code) both exist and still expose `./skills/`.
2. Confirm `./.agents/plugins/marketplace.json` still points to `./plugins/repo-skills` for Codex, and any Claude Code marketplace entry the user maintains still points to the same plugin path.
...
- `SKILL.md` required
- `agents/openai.yaml` recommended only when the skill needs Codex marketplace metadata (display name, short description, default prompt). Claude Code reads `SKILL.md` frontmatter directly and has no analogous per-skill file.
...
3. Make `SKILL.md` tell the agent to read `WORKFLOW.md`.
4. Add `agents/openai.yaml` with a human-facing display name, short description, and default prompt when Codex marketplace metadata is needed; skip it when only Claude Code discovery matters.
...
- Do not store persistent state in user-global plugin or skill caches (such as `~/.codex/skills` or `~/.claude/plugins/`), other plugin cache folders, temp files under the home directory, or absolute per-user paths.
...
If the skill updates client-facing discovery behavior, keep the public skill names unique within the plugin.

## Discovery And User Guidance

After adding or updating the skill, tell the user what their agent client (Codex or Claude Code) needs in order to discover it.

Use these rules:

- If the plugin is not currently installed for the workspace, tell the user to install `Repo Skills` from the repo-local marketplace and then restart their agent client. In Claude Code that is `/plugin marketplace add` followed by `/plugin install`; in Codex it is the marketplace install flow.
- If the plugin is already installed and you changed the exposed skill set or plugin metadata, tell the user to restart their agent client.
- If a restart still does not show the new skill, tell the user to uninstall the plugin, restart the client, install the plugin again, and restart once more.
- If the currently running client is using a cached installed copy and immediate validation matters, update the live cache copy too when it is safe to do so, then still tell the user a restart is needed.

Do not tell the user to activate or copy the skill into user-global plugin or skill caches.

## Validation

Validate by inspection unless the user asks for more:

1. Confirm both plugin manifests (`.codex-plugin/plugin.json` and `.claude-plugin/plugin.json`) still point to `./skills/`.
...
5. Confirm the repo-local marketplace entries still point at `./plugins/repo-skills`.
