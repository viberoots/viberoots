...
While implementation subagents are waiting on focused or full-suite validation, require concise progress updates from the `test` skill workflow and relay useful updates to the user during long waits. Validation should run as a logged background process that the subagent polls, not as one blocking foreground command. Updates should include elapsed time, log path, current phase, or a short high-signal summary. Keep verbose build and test output in the log file, and avoid empty updates.

Do not treat the start of a validation run as completion. A subagent must wait for the test process to exit before reporting. The reliable completion signal is the `test` skill / `i && b && v` invocation returning a final exit code — not a string in the log. The repo's verify pipeline writes `Tests finished:` once per phase (e.g. shared, then resource-limited), so a single occurrence does not mean the run is done; only process exit does. Prefer a logged background validation process with PID polling so progress can be reported, and wait for that PID to exit before producing final evidence.
...
While tests or full-suite validation are running, keep the full output in a log file and send concise progress updates about every 60-120 seconds or when the phase changes. Do not run validation as one blocking foreground command; start the logged validation process in the background, poll its PID, and use that polling to report progress. Each update should include elapsed time, the log path, and useful state such as the current phase or a short high-signal summary; do not paste verbose logs. DO NOT end your turn until the test process has exited. Do not treat the start of a run as completion, and do not treat a `Tests finished:` line as completion — that line is emitted once per phase (shared, then resource-limited), so multiple occurrences are normal and a single occurrence does not mean the run is done. The only reliable completion signal is the `test` / `i && b && v` invocation returning a final exit code. Report completion only after the final exit code is observed. Every focused or full-suite report must include the absolute log path, the final exit code, the per-phase `Tests finished:` summary lines from the log, and the total elapsed time in seconds.
