...
For each PR number in the range, spawn a fresh implementation subagent and instruct it to run the `pr` skill for exactly that PR number and plan document. Before authorizing a commit, spawn a separate reviewer subagent to compare the implementation against the PR's plan-document scope. Advance to the next PR only after the current PR reports that all self-reviews, focused validation, full-suite validation, the full-suite timing-regression check, the scope review, and the commit succeeded. After the current range completes, assess the plan and any known design document; if gaps remain, augment the plan with follow-up PRs, extend the range, update progress, and keep working.
...
3. Tell the subagent to use the `pr` skill for the exact PR number and plan document.
...
5. Explicitly ask the subagent to check the successful full-suite execution time against that baseline.
6. Explicitly ask the subagent to stop before committing and report ready-for-review evidence after the `pr` skill reports that self-review, focused validation, full-suite validation, failure investigations, and the timing-regression check all pass.
...
9. If the reviewer finds incomplete scope, send the findings back to the implementation subagent and tell it to finish the missing work, rerun the required `pr` validation and timing checks, and report ready-for-review evidence again.
10. Repeat implementation, validation, and review until the reviewer passes or the PR is genuinely blocked.
11. After the reviewer passes, tell the implementation subagent to use the `cc` skill to create the conventional commit for this PR and report commit evidence.
12. Wait for that subagent to finish before starting the next PR.
...
Use the `pr` skill for PR <pr-number> from <plan-document>.

Because this is a `$prs` run, after the `pr` workflow reports that self-review, focused validation, full-suite validation, and any investigations all pass, compare the successful full-suite total execution time against this baseline:
...
After the full suite passes without a significant timing jump, stop before committing and report ready-for-review evidence. A separate reviewer subagent must pass the implementation against the PR description in the plan document before a commit is authorized.

If the reviewer reports missing scope, finish the missing work, rerun the required `pr` validation and timing checks, and report ready-for-review evidence again.

Only after `$prs` tells you the reviewer passed, use the `cc` skill to create the conventional commit representing exactly this PR's changes. Do not commit by running raw `git commit` yourself unless the `cc` skill is unavailable and the user explicitly authorizes a fallback.
...
Before authorizing a commit for each PR, spawn one fresh reviewer subagent.
...
- Assume tests are already passing. Do not run tests or validation commands.
- Do not modify files.
...
Assume tests and validation are already passing. Do not run tests, builds, formatters, or validation commands. Do not modify files.
...
If the scope review fails, do not authorize a commit. Send the findings to the implementation subagent and tell it to complete the missing PR scope. After any implementation change, require the implementation subagent to rerun the `pr` workflow's focused validation, full-suite validation, timing-regression check, and failure investigation loop before another scope review.
...
- A separate scope-review subagent passed the implementation against the PR description in the plan document.
...
- scope-review evidence, including whether the reviewer requested follow-up work before passing
...
- Do not authorize a commit without a passing scope review from a separate reviewer subagent.
- Do not let the reviewer subagent run tests or modify files; it reviews scope only and assumes validation has already passed.
