...
def _rust_nix_build_impl(ctx):
    ...
    safe_log_path_prefix = (
        ...
        + "SAFE_LOG_KEY=\"${SAFE_LOG_KEY//:/_}\"; "
        + "BUILD_SELECTED_LOG=\"$WORKSPACE_ROOT/buck-out/tmp/build-selected/rust_nix_build.${SAFE_LOG_KEY}.log\"; "
    ...
    run_and_copy = (
        ...
        + "CAND=\"\"; "
        + "for c in \"$outPath/bin/$TARGET_NAME\" \"$outPath/bin/$SANITIZED\" \"$outPath/bin/rust-$SANITIZED\"; do "
