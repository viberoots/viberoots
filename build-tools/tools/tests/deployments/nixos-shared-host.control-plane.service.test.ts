  ...
  await runInTemp("nixos-shared-host-control-plane-service-cli", async (tmp, $) => {
    ...
    try {
      const result = await $({
        ...
        stdio: "pipe",
      })`zx-wrapper ${viberootsToolScript("build-tools/tools/deployments/deploy.ts")} --deployment ${deploymentLabel} --artifact-dir ${artifactDir} --admission-evidence-json ${admissionEvidenceJson} --control-plane-url ${controlPlane.url} --smoke-connect-host 127.0.0.1 --smoke-connect-port ${String(server.port)} --smoke-connect-protocol https:`;
