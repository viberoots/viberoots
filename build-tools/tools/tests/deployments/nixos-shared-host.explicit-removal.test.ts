...
test("nixos-shared-host deploy CLI records explicit removal and cleans up the realized target", async () => {
  await runInTemp("nixos-shared-host-explicit-removal", async (tmp, $) => {
    ...
    try {
      ...
      const removal = await $({
        cwd: tmp,
      })`zx-wrapper ${viberootsToolScript("build-tools/tools/deployments/deploy-internal.ts")} --deployment ${deployment.label} --admission-evidence-json ${admissionEvidenceJson} --control-plane-url ${harness.controlPlane.url} --remove`;
