...
async function runDeployCli(
  ...
  return await $({
    ...
    stdio: "pipe",
  })`zx-wrapper ${viberootsToolScript("build-tools/tools/deployments/deploy.ts")} --deployment ${deploymentLabel} --admission-evidence-json ${admissionEvidenceJson} ${extraArgs}`.nothrow();
}
