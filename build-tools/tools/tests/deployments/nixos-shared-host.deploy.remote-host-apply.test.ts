...
test("remote deploy rejects legacy host-apply mutation flags in service-only mode", async () => {
  await runInTemp("nixos-shared-host-remote-host-apply-rejected", async (tmp, $) => {
    ...
    })`zx-wrapper ${viberootsToolScript("build-tools/tools/deployments/deploy.ts")} --deployment ${REVIEWED_SAMPLE_WEBAPP_DEPLOYMENT_LABEL} --admission-evidence-json ${admissionEvidencePath} --profile mini --profile-root ${profileRoot} --artifact-dir ${artifactDir} --apply-host`.nothrow();
...
test("remote deploy rejects host-apply path overrides in service-only mode", async () => {
  await runInTemp("nixos-shared-host-remote-host-apply-overrides-rejected", async (tmp, $) => {
    ...
    const result = await $({
      cwd: tmp,
      env: remoteExecEnv(env),
    })`zx-wrapper ${viberootsToolScript("build-tools/tools/deployments/deploy.ts")} --deployment ${REVIEWED_SAMPLE_WEBAPP_DEPLOYMENT_LABEL} --admission-evidence-json ${admissionEvidencePath} --profile mini --profile-root ${profileRoot} --artifact-dir ${artifactDir} --remote-config-root /srv/nixos`.nothrow();
    assert.notEqual(result.exitCode, 0);
    assert.match(String(result.stderr), /service-only remote profiles do not support/);
  });
});
