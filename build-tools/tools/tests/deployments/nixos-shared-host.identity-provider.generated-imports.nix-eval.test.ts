...
test("shared-host identity provider module bootstraps generated identity imports", async () => {
  ...
  await runInTemp("shared-host-identity-provider-generated-imports-eval", async (tmp, $) => {
    ...
    assert.match(out.bootstrapScript, /"claim\.value":"viberoots\/viberoots"/);
    ...
    assert.doesNotMatch(out.keycloakPreStart, /cmp -s "\$generated_realm_tmp"/);
    ...
    assert.doesNotMatch(out.keycloakPostStart, /-q search="\$group_name"/);
