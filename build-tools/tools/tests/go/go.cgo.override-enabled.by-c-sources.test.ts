...
load("@viberoots//build-tools/go:defs.bzl", "nix_go_test")
...
EOF'`;
    ...
    const probe = await $({
      ...
      reject: false,
      nothrow: true,
    })`buck2 --isolation-dir ${inheritedBuckIsolation("go_cgo_override_srcs_test")} cquery --target-platforms //:no_cgo --json --output-attribute override_cgo_enabled --output-attribute labels //tmp:t`;
