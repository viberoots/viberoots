...
test("build-tools/node/defs_nix.bzl must not reintroduce local importer normalization helpers", async () => {
  ...
  assert(
    ...
    `${file} must not define _sanitize_importer_attr(...); use //lang:importer_strings.bzl`,
  );
  assert(
    !txt.includes("def _basename_importer("),
    `${file} must not define _basename_importer(...); use //lang:importer_strings.bzl`,
  );

  assert(
    txt.includes('load("//lang:importer_strings.bzl",'),
    `${file} must load //lang:importer_strings.bzl`,
  );
  assert(
