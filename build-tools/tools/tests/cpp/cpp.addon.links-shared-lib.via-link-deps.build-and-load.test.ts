...
test("cpp Node-API addon links a shared C++ lib via link_deps (build + load)", async () => {
  await runInTemp("cpp-addon-links-shared-lib", async (tmp, $) => {
    await fs.outputFile(
    ...
    await fs.outputFile(
      ...
      ['#include "../include/shared.h"', "int shared_answer() { return 7; }", ""].join("\n"),
      "utf8",
    );
    await fs.outputFile(
    ...
    await fs.outputFile(
      ...
      [
        ...
        "  napi_value num;",
        "  napi_create_int32(env, shared_answer(), &num);",
        "  return num;",
        "}",
        "",
    ...
    const probe = await $({
      ...
      reject: false,
      nothrow: true,
    })`buck2 --isolation-dir ${inheritedBuckIsolation("cpp_addon_shared")} cquery "deps(//projects/libs/addon-native:addon)" --json --output-attribute name`;
    ...
    const script = [
      ...
      "const v = a.answer();",
      "if (v !== 7) { console.error('bad answer', v); process.exit(3); }",
      "console.log('ok');",
    ].join("");
    const run = await $({
      cwd: tmp,
