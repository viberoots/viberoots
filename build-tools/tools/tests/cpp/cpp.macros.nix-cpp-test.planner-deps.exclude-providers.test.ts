...
test("nix_cpp_test planner stub deps exclude //third_party/providers:* targets", async () => {
  await runInTemp("cpp-nix-cpp-test-planner-deps", async (tmp, $) => {
    ...
    await fsp.mkdir(path.join(tmp, "apps", "demo", "patches", "cpp"), { recursive: true });
    ...
    const q = await $({
      cwd: tmp,
      stdio: "pipe",
    })`buck2 cquery --target-platforms //:no_cgo //apps/demo:demo_test__planner --json --output-attribute deps`;
    ...
    assert.ok(deps.includes("//apps/demo:helper"), "expected helper dep on planner stub");
    assert.ok(
    ...
    const s = await $({
      cwd: tmp,
      stdio: "pipe",
    })`buck2 cquery --target-platforms //:no_cgo //apps/demo:demo_test__planner --json --output-attribute srcs`;
