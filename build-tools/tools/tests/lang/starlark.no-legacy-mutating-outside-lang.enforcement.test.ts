...
test("no legacy mutating wiring helpers are referenced outside //lang/*", async () => {
  ...
  for (const file of bzlFiles) {
    ...
    assert.ok(
      ...
      `${file} must not reference *_legacy_mutating helpers; use //lang shared wiring helpers instead`,
    );
  }
});
