...
test("labels-probe: cpp emscripten stamps viberoots/build-tools/lang/kind/variant", async () => {
  await runInTemp("probe-cpp-ems", async (tmp, $) => {
    ...
    await fs.writeFile(
      ...
      [
        'load("@viberoots//build-tools/lang:defs_common.bzl", "wasm_labels_probe")',
        "",
        "wasm_labels_probe(",
        '  name = "cpp_ems",',
        '  lang = "cpp",',
