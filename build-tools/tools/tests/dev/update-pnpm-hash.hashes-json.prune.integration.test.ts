...
test("pruneNodeModulesHashesJson removes stale lockfile keys", async () => {
  ...
  try {
    process.chdir(tmp);
    const hashesPath = path.join("projects", "config", "node-modules.hashes.json");
...
test("pruneNodeModulesHashesJson prunes workspace hashes when invoked below root", async () => {
  ...
  try {
    ...
    const hashesPath = path.join(projectsDir, "config", "node-modules.hashes.json");
    await fsp.mkdir(path.dirname(hashesPath), { recursive: true });
    await fsp.writeFile(
      hashesPath,
      JSON.stringify(
    ...
    await assert.rejects(
      fsp.stat(path.join(projectsDir, "projects", "config", "node-modules.hashes.json")),
    );
  } finally {
    process.chdir(prevCwd);
    await fsp.rm(tmp, { recursive: true, force: true });
...
test("hash JSON reads and writes use explicit workspace root below root", async () => {
  ...
  try {
    ...
    const rootHashesPath = path.join(tmp, "projects", "config", "node-modules.hashes.json");
    const nestedHashesPath = path.join(
      projectsDir,
      "projects",
      "config",
...
test("pruneNodeModulesHashesJson does not mutate extracted viberoots source hashes", async () => {
  ...
  try {
    ...
    const consumerHashesPath = path.join("projects", "config", "node-modules.hashes.json");
...
test("updateNodeModulesHashesJson writes consumer root hashes to projects ownership", async () => {
  ...
  try {
    ...
    const next = JSON.parse(
      await fsp.readFile(path.join("projects", "config", "node-modules.hashes.json"), "utf8"),
...
test("updateNodeModulesHashesJson does not write through activated remote source", async () => {
  ...
  try {
    ...
    await assert.rejects(
      fsp.stat(path.join(source, "projects", "config", "node-modules.hashes.json")),
    );
  } finally {
    process.chdir(prevCwd);
    await fsp.rm(tmp, { recursive: true, force: true });
...
test("viberoots-owned hash writes fall back to workspace hashes for activated remote source", async () => {
  ...
  try {
    ...
    const consumerHashesPath = path.join(
      consumer,
      "projects",
      "config",
      "node-modules.hashes.json",
...
test("updateNodeModulesHashesJson keeps standalone viberoots hashes in tooling ownership", async () => {
  ...
  try {
    ...
    await assert.rejects(fsp.stat(path.join("projects", "config", "node-modules.hashes.json")));
  } finally {
    process.chdir(prevCwd);
    await fsp.rm(tmp, { recursive: true, force: true });
  }
...
test("updateNodeModulesHashesJson writes project hashes to projects ownership", async () => {
  ...
  try {
    ...
    const projectsHashesPath = path.join(tmp, "projects", "config", "node-modules.hashes.json");
