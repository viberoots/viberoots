...
test("install node patch warning surfaces importer-specific remediation command", async () => {
  await runInTemp("install-node-patch-warning-command", async (tmp, $) => {
    ...
    })`node --input-type=module --experimental-strip-types --import ./build-tools/tools/dev/zx-init.mjs -e "import { warnNodePatchRequirementsInLocal } from './build-tools/tools/lib/node-deps-enforcement.ts'; await warnNodePatchRequirementsInLocal(process.cwd());"`;
