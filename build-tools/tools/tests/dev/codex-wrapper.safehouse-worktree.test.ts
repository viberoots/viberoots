...
test("codex --worktree creates through the CoW git wrapper and launches the worker in safehouse", async () => {
  ...
  try {
    ...
    assert.match(log, /safehouse --enable=keychain /);
    assert.match(log, new RegExp(`--env ${escapeRegExp(path.join(fake.bin, "codex"))} `));
    assert.doesNotMatch(log, /Codex\.app|codex-execve-wrapper|\.codex\/tmp\/arg0/);
    ...
    assert.match(
      log,
      /\(allow mach-lookup \(global-name "com\.apple\.securityd"\) \(global-name "com\.apple\.sysmond"\)\)/,
    );
