...
test(
  "phase6: node/cpp/rust parity signals hold in a minimal environment",
  { timeout: TEST_TIMEOUT_MS },
  async () => {
    await runInTemp("nix-gaps-phase6-parity", async (tmp, $) => {
      const nodeImporter = "projects/apps/phase6-node";
      ...
      await fs.outputFile(
        path.join(tmp, "projects", "apps", "phase6-cpp", "src", "main.cpp"),
        '#include <iostream>\nint main(){ std::cout<<"cpp-phase6\\n"; return 0; }\n',
        "utf8",
      );

      await fs.outputFile(
        path.join(tmp, "projects", "apps", "phase6-rust", "src", "main.rs"),
        "fn main() {}\n",
        "utf8",
      );
      ...
      await fs.outputFile(
        ...
        JSON.stringify(
          [
            {
              name: "//projects/apps/phase6-node:gen_bin",
              rule_type: "genrule",
            ...
            {
              name: "//projects/apps/phase6-cpp:app",
              rule_type: "cxx_binary",
              labels: ["lang:cpp", "kind:bin"],
              srcs: ["projects/apps/phase6-cpp/src/main.cpp"],
            },
            {
              name: "//projects/apps/phase6-rust:app",
              rule_type: "rust_binary",
              labels: ["lang:rust", "kind:bin"],
              srcs: ["projects/apps/phase6-rust/src/main.rs"],
            },
          ],
          null,
          2,
      ...
      const expectedNodeHash = sha256Hex("node-phase6\n");
      const cases: BuildCase[] = [
        {
          target: "//projects/apps/phase6-node:gen_bin",
          validate: async (outPath) => {
            const outFile = path.join(outPath, "bin", "phase6-node.sh");
            const txt = await fs.readFile(outFile, "utf8");
            assert.equal(sha256Hex(txt), expectedNodeHash);
          },
        },
        {
          target: "//projects/apps/phase6-cpp:app",
          validate: async (outPath, _$) => {
            ...
            assert.equal(String(run.stdout || "").trim(), "cpp-phase6");
          },
        },
        {
          target: "//projects/apps/phase6-rust:app",
          validate: async (outPath, _$) => {
