...
test("probe-graph-generator-selected defaults to short timeout for fast repro", async () => {
  ...
  if (!txt.includes("BNX_RUNNABLE_BUILD_TIMEOUT_SEC")) {
    throw new Error(`${file} must set BNX_RUNNABLE_BUILD_TIMEOUT_SEC when missing`);
  }
  if (!txt.includes('process.env.BNX_RUNNABLE_BUILD_TIMEOUT_SEC = "25"')) {
