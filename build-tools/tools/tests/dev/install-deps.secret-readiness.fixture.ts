...
type ReadinessOptions = Parameters<typeof ensureInstallSecretReadiness>[0];
...
export async function ensureReadiness(
  repoRoot: string,
  options: Pick<ReadinessOptions, "flags" | "deps">,
): Promise<void> {
...
export function nonEmptyResetPlan() {
  return {
...
export async function captureStderr(fn: () => Promise<void>) {
...
export async function withRepo(fn: (repoRoot: string) => Promise<void>) {
...
export async function writeResolver(repoRoot: string) {
  const config = {
...
export async function writeCredentials(repoRoot: string) {
  const refs = [
