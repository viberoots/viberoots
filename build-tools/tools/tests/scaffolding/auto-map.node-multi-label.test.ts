...
test("auto-map maps only lockfile providers when both module + lockfile labels present", async () => {
  await runInTemp("auto-map-multi", async (tmp, $) => {
    ...
    const graphContent = JSON.stringify([
      {
        ...
        labels: [
          ...
          "lockfile:projects/apps/hybrid/pnpm-lock.yaml#projects/apps/hybrid",
          "lang:go",
          "lang:node",
        ],
    ...
    // Create lockfile
    const lockfilePath = path.join(tmp, "projects", "apps", "hybrid", "pnpm-lock.yaml");
    await fsp.mkdir(path.dirname(lockfilePath), { recursive: true });
    await fsp.writeFile(
      lockfilePath,
      `lockfileVersion: "9.0"\nimporters:\n  projects/apps/hybrid:\n    dependencies: {}\npackages: {}`,
      "utf8",
    );
    ...
    if (!autoMapContent.includes("//projects/apps/hybrid:service")) {
      console.error("Missing //projects/apps/hybrid:service in auto_map");
      process.exit(2);
    }

    // Should have only lockfile provider (no module provider)
    // Extract the providers for this target
    const targetMatch = autoMapContent.match(
      /"\/\/projects\/apps\/hybrid:service":\s*\[([\s\S]*?)\]/,
    );
    if (!targetMatch) {
      console.error("Could not find provider list for //projects/apps/hybrid:service");
      process.exit(2);
    }
