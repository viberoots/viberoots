...
async function expectMissingArtifactFailure(
  ...
  const sandbox = path.join(scratchRoot, `vite-missing-${missing}`);
...
test(
  "Vite SSR contracts: packaging shape, runtime startup, and missing-artifact failures",
  { timeout: TEST_TIMEOUT_MS },
  async () => {
    await withTempRoots(async () => {
      await runInTemp("node-webapp-ssr-vite-contracts", async (tmp, _$) => {
