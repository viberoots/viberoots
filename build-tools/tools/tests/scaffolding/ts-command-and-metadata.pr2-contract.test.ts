...
test("PR-2 command surface: TypeScript templates are ts-only", async () => {
  await runInTemp("scaf-pr2-ts-only", async (_tmp, _$) => {
    const $ = _$({ stdio: "pipe" });
...
test("PR-2 metadata contract: canonical ts templates have ts language and help fields", async () => {
