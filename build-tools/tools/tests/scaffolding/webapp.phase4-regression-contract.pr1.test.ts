...
test("phase4 pr1 stale install-lock contract removes orphaned lock deterministically", async () => {
  const scopeRootAbs = await fsp.mkdtemp(path.join(os.tmpdir(), "phase4-pr1-lock-"));
  const key = `phase4-pr1-${Date.now()}-${process.pid}`;
...
test("phase4 pr1 watcher failure helper enforces deterministic failure signature", async () => {
  const okLogs = [
...
test("phase4 pr1 missing local-link helper reports deterministic recovery guidance", async () => {
