...
test(
  "webapp-ssr-next runtime contract supports generated multi-module manifests",
  ...
  async () => {
    await runInTemp("webapp-ssr-next-multi-module-runtime-contract", async (tmp, _$) => {
      ...
      const appWasmHelper = await fsp.readFile(
        path.join(appAbs, "app", "wasm-contract.ts"),
        "utf8",
      );
      ...
      try {
        await waitForValue(
          async () =>
            await fsp.readFile(path.join(appAbs, "app", "wasm-contract", "top.wasm"), "utf8"),
