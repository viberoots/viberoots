...
test("repo_cgo_deps wires local cpp lib and runs", async () => {
  await runInTemp("go-cgo-repo-lib", async (tmp, $) => {
    ...
    await $({
      cwd: tmp,
    })`bash --noprofile --norc -c 'mkdir -p projects/libs/greeter/include projects/libs/greeter/src projects/apps/demo-cli/cmd/demo'`;
    await $({
      cwd: tmp,
    })`bash --noprofile --norc -c 'cat > projects/libs/greeter/include/greeter.h <<"EOF"\n#pragma once\n#ifdef __cplusplus\nextern "C" {\n#endif\nconst char* greet();\n#ifdef __cplusplus\n}\n#endif\nEOF'`;
    await $({
      cwd: tmp,
    })`bash --noprofile --norc -c 'cat > projects/libs/greeter/src/greeter.cpp <<"EOF"\n#include <string>\n#include "greeter.h"\nstatic std::string s = std::string("hello from cpp");\nconst char* greet() { return s.c_str(); }\nEOF'`;
    ...
    await $({
      cwd: tmp,
    })`bash --noprofile --norc -c 'cat > projects/apps/demo-cli/cmd/demo/main.go <<"EOF"\npackage main\n/*\n#cgo LDFLAGS: -lstdc++\n#include "greeter.h"\n*/\nimport "C"\n\nimport "fmt"\n\nfunc main() {\n  fmt.Println("greet:", C.GoString(C.greet()))\n}\nEOF'`;
