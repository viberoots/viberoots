...
test("move, update, delete and ls reflects state", async () => {
  await runInTemp("scaf-e2e", async (_tmp, _$) => {
    ...
    await $`scaf update projects/libs/demo-moved --dry-run`;
    let prompted = false;
    try {
      await $`scaf update projects/libs/demo-moved`;
    } catch {
      prompted = true;
    }
    if (!prompted) {
    ...
    await $`scaf update projects/libs/demo-moved --yes`;
    await $`scaf delete projects/libs/demo-moved --dry-run`;
    await $`scaf delete projects/libs/demo-moved --yes`;
    const res = await pipe$`scaf ls --json`;
    const arr = JSON.parse(res.stdout.trim() || "[]");
    if (arr.some((r: any) => String(r.path || "").endsWith("projects/libs/demo-moved"))) {
