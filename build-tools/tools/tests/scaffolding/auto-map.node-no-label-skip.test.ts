...
test("auto-map skips targets without lockfile labels", async () => {
  await runInTemp("auto-map-skip", async (tmp, $) => {
    ...
    const graphContent = JSON.stringify([
      ...
      {
        name: "//projects/libs/cpp-only:lib",
      ...
      {
        name: "//projects/apps/node:bundle",
        rule_type: "genrule",
        labels: ["lockfile:projects/apps/node/pnpm-lock.yaml#projects/apps/node", "lang:node"],
        srcs: ["index.ts"],
        deps: [],
      },
    ]);
    ...
    const lockfilePath = path.join(tmp, "projects/apps/node/pnpm-lock.yaml");
    ...
    await $`git add projects/apps/node/pnpm-lock.yaml`;
    ...
    if (!autoMapContent.includes("//projects/apps/node:bundle")) {
      console.error("Expected //projects/apps/node:bundle in auto_map");
      process.exit(2);
    }

    const nodeBundleMatch = autoMapContent.match(
      /"\/\/projects\/apps\/node:bundle":\s*\[([\s\S]*?)\]/,
    );
    if (nodeBundleMatch && nodeBundleMatch[1].includes("lf_")) {
      // Good - has lockfile provider
    } else {
      console.error("Expected lockfile provider for //projects/apps/node:bundle");
      process.exit(2);
    }

    // Go target should NOT have a lockfile provider (only module provider)
    if (autoMapContent.includes("//projects/apps/go-only:service")) {
      const goMatch = autoMapContent.match(
        /"\/\/projects\/apps\/go-only:service":\s*\[([\s\S]*?)\]/,
      );
