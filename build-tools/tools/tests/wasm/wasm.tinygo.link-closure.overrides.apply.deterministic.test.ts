...
test(
  "wasm: tinygo per-dep link closure overrides are observable and deterministic",
  ...
  async () => {
    await runInTemp("wasm-tinygo-link-closure-overrides", async (tmp, $) => {
      const mkLib = async (dirName: string, name: string, opts?: { linkDeps?: string[] }) => {
        ...
        await fs.outputFile(
          path.join(dir, "include", `${dirName}.h`),
          `int ${dirName}_id(void);\n`,
        );
        await fs.outputFile(
      ...
      if (!libLabels || !overrides)
        throw new Error("missing build.log diagnostics for wasm linkage");
