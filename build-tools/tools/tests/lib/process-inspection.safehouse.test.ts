...
test("process inspection prefers pgrep inside agent Safehouse sessions", () => {
  assert.equal(processInspectionPrefersPgrep({ BNX_CODEX_SAFEHOUSE_ACTIVE: "1" }), true);
  assert.equal(processInspectionPrefersPgrep({ BNX_CLAUDE_SAFEHOUSE_ACTIVE: "1" }), true);
  assert.equal(processInspectionPrefersPgrep({}), false);
});

test("process start signatures do not invoke ps inside Safehouse", async () => {
  const oldCodex = process.env.BNX_CODEX_SAFEHOUSE_ACTIVE;
  process.env.BNX_CODEX_SAFEHOUSE_ACTIVE = "1";
  try {
    assert.equal(await processStartSignature(process.pid), null);
  } finally {
    if (oldCodex === undefined) {
      delete process.env.BNX_CODEX_SAFEHOUSE_ACTIVE;
    } else {
      process.env.BNX_CODEX_SAFEHOUSE_ACTIVE = oldCodex;
    }
  }
});
