...
const profilesPath = "toolchains/remote_execution_profiles.bzl";
const platformsPath = "toolchains/remote_execution_platforms.bzl";
...
test("dormant remote execution platforms analyze with explicit executor policy", async () => {
  ...
  assert.match(providers.stdout, /data: "buck2-build"/);
...
test("ordinary execution-platform resolution stays local unless config opts in", async () => {
  const local = await $({
    stdio: "pipe",
  })`buck2 audit execution-platform-resolution //:flake.lock`.nothrow();
  ...
  const remote = await $({
    stdio: "pipe",
  })`buck2 audit execution-platform-resolution -c build.execution_platforms=repo_toolchains//:remote_execution_platforms //:flake.lock`.nothrow();
