...
test("node adapter accepts repeated './' normalization in lockfile labels (warn mode)", async () => {
  await runInTemp("exp-node-warn-dot-slash-ok", async (tmp, $) => {
    ...
    if (txt.includes("malformed lockfile label") || txt.includes("lockfile importer mismatch")) {
      console.error("expected no lockfile label warnings for dot-slash-normalized label", txt);
      process.exit(2);
    }
  });
});
