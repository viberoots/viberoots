...
const BIN_ROOT = path.join(REPO_ROOT, "build-tools", "tools", "bin");
...
async function runBareCommands(consumer: string, cwd: string): Promise<void> {
  ...
  await $({ cwd, env, stdio: "pipe" })`${path.join(BIN_ROOT, "i")} --glue-only --skip-go-tidy`;
  assert.equal(
    await exists(
  ...
  await $({
    ...
    env,
    stdio: "pipe",
  })`${path.join(BIN_ROOT, "b")} build --no-materialize //projects/apps/demo:smoke_script`;
  await $({
    cwd,
    env,
    stdio: "pipe",
  })`${path.join(BIN_ROOT, "v")} //projects/apps/demo:smoke_test`;
  await $({
    cwd,
    env: { ...env, VBR_TAIL_LOG_STATUS_INTERVAL: "1" },
    stdio: "pipe",
    nothrow: true,
  })`timeout 3s ${path.join(BIN_ROOT, "s")}`;
}

test("committed remote consumer template pins an explicit release flake lock", async () => {
  ...
  assert.match(await fsp.readFile(flakePath, "utf8"), /github:viberoots\/viberoots\/v1\.4\.2/);
  const lock = JSON.parse(await fsp.readFile(lockPath, "utf8"));
  assert.equal(lock.nodes.viberoots.original.type, "github");
  assert.equal(lock.nodes.viberoots.original.owner, "viberoots");
  assert.equal(lock.nodes.viberoots.original.repo, "viberoots");
  assert.equal(lock.nodes.viberoots.original.ref, "v1.4.2");
});
...
test(
  ...
  async () => {
    await runInScratchTemp("viberoots-real-remote-consumer", async (tmp, $) => {
      ...
      const expected = new RegExp(`^${realRemoteRef.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`);
