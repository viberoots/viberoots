...
export function buildVerifyTestEnvArgs(opts: VerifyBuck2TestEnvArgsOptions): string[] {
  ...
  return [
    ...
    "--env",
    `BNX_AGENT_SAFEHOUSE_E2E=${process.env.BNX_AGENT_SAFEHOUSE_E2E || ""}`,
    "--env",
    `BNX_APFS_CLONE_CHECKER=${process.env.BNX_APFS_CLONE_CHECKER || ""}`,
    ...maybeEnvArg(
      "BNX_AGENT_SAFEHOUSE_E2E_PATH",
      process.env.BNX_AGENT_SAFEHOUSE_E2E === "1" ? process.env.PATH : undefined,
    ),
    "--env",
