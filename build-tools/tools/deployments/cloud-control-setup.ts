...
function runtimeInputFromFlags(): CloudControlSetupInput["runtimeInput"] {
  const filePath = getFlagStr("runtime-input", "").trim();
  if (filePath) return readRuntimeInputFile(filePath);
  return undefined;
}
