...
const ALLOWED_INFISICAL_RUNTIME_KEYS = new Set([
  "site_url",
  "project_id",
  "environment",
  "secret_path",
...
export function pushForbiddenInfisicalRuntimeKeyErrors(opts: {
  ...
  for (const key of Object.keys(opts.runtime)) {
    if (!ALLOWED_INFISICAL_RUNTIME_KEYS.has(key)) {
      opts.errors.push(deploymentError(opts.label, `infisical_runtime.${key} is unsupported`));
    }
  }
}
