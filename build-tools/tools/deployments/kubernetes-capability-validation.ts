...
export function pushKubernetesComponentKindErrors(opts: {
  label: string;
  declaredKind: string;
  ...
  if (!providerSupportsComponentKind("kubernetes", opts.declaredKind)) {
    opts.errors.push(
      deploymentError(
        opts.label,
        `kubernetes provider capability does not support component_kind "${opts.declaredKind}"`,
      ),
    );
  }
