...
import type { GooglePlayControlPlaneSnapshot } from "./google-play-control-plane";
...
export async function executeGooglePlayControlPlaneSubmission(opts: {
  ...
  ) as GooglePlayControlPlaneSnapshot;
  requireFrozenProviderSnapshot(snapshot, "google-play");
  requireFrozenProviderSubmissionAdmission({ provider: "google-play", submission, snapshot });
  if (snapshot.operationKind !== "deploy")
    requireFrozenProviderReplaySource(snapshot, "google-play");
  ...
  const result = await publishRecordedGooglePlayArtifact({
