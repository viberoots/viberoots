...
export function credentialCommands(input: CloudControlSetupInput): RunbookCommand[] {
  return [
    credentialCommand(
      ...
      `${rootPrelude(input.outDir)}; control-plane credential-preflight --bundle-dir "$PROFILE_ROOT" --out "$PROFILE_ROOT/credential-preflight.json"`,
      ["$PROFILE_ROOT/credential-preflight.json"],
      "credential manifest and files match",
    ),
    credentialCommand(
      "credential-staging",
      `${rootPrelude(input.outDir)}; control-plane credential-staging --bundle-dir "$PROFILE_ROOT" --out "$PROFILE_ROOT/credential-staging.json"`,
    ...
    credentialCommand(
      "credential-rotation",
      `${rootPrelude(input.outDir)}; control-plane credential-rotation --bundle-dir "$PROFILE_ROOT" --apply-rotation --out "$PROFILE_ROOT/credential-rotation.json" --rotated-map-out "$PROFILE_ROOT/credential-map.rotated.json"`,
