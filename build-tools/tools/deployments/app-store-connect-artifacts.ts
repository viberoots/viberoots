...
export type AdmittedMobileAppArtifact = {
  kind: "mobile-app";
  identity: string;
...
async function resolveArtifactFile(inputPath: string): Promise<string> {
  ...
  if (matches.length !== 1) {
    throw new Error(`mobile-app artifact path must resolve to exactly one .ipa file: ${inputPath}`);
  }
  return matches[0];
}
...
export async function admitMobileAppArtifact(opts: {
  recordsRoot: string;
  artifactPath: string;
}): Promise<AdmittedMobileAppArtifact> {
  ...
  const artifact: AdmittedMobileAppArtifact = {
    kind: "mobile-app",
    identity,
  ...
  if (!(await pathExists(artifact.provenancePath))) {
    ...
    await fsp.writeFile(
      ...
      JSON.stringify(
        {
          schemaVersion: APP_STORE_CONNECT_ARTIFACT_PROVENANCE_SCHEMA,
...
export async function requireAdmittedMobileAppArtifactPath(
