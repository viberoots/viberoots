...
type PromotionSourceRecord = CloudflarePagesDeployRecord | NixosSharedHostDeployRecord;
type PromotionSourceReplaySnapshot = CloudflarePagesReplaySnapshot | NixosSharedHostReplaySnapshot;

type PromotionSource = {
  record: PromotionSourceRecord;
  recordPath: string;
  replaySnapshot: PromotionSourceReplaySnapshot;
...
function sourceStage(source: PromotionSource): string {
  return source.replaySnapshot.deployment.environmentStage;
}

function sourceRevision(source: PromotionSource): string {
  return source.replaySnapshot.admittedContext.source.sourceRevision;
}

function replaySnapshotPath(source: PromotionSource): string {
  return source.replaySnapshotPath;
}
...
async function resolvePromotionSourceByPath(recordPath: string): Promise<PromotionSource> {
...
async function resolvePromotionSource(opts: {
...
export async function resolveCloudflarePagesPromotionSelection(opts: {
  ...
  sourceReplaySnapshot: PromotionSourceReplaySnapshot;
}> {
  const source = await resolvePromotionSource(opts);
  const errors = [
...
${errors.join("\n")}`);
  ...
  return {
    ...
    sourceReplaySnapshotPath: replaySnapshotPath(source),
