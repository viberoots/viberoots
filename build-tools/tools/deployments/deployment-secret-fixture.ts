...
export async function readDeploymentSecretFixture(): Promise<DeploymentSecretFixture> {
  ...
  if (!filePath) {
    throw new Error(
      `secret-consuming protected/shared runs require either ${DEPLOYMENT_SECRET_FIXTURE_PATH_ENV} for the reviewed local/test secret fixture or VAULT_ADDR plus VAULT_TOKEN for direct Vault runtime`,
    );
  }
