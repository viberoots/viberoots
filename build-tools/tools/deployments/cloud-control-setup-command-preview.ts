...
function setupCommand(input: CloudControlSetupInput, dryRun: boolean): string {
  ...
  const args = [
    ...
    "--host-mode",
    input.mode,
    input.mode === "aws-ec2" ? "--ec2-host-mode" : undefined,
    input.mode === "aws-ec2" ? input.ec2HostMode || "external-reviewed-host" : undefined,
    input.ec2HostMode === "repo-owned-asg" ? "--ec2-asg-evidence-mode" : undefined,
    input.ec2HostMode === "repo-owned-asg" ? input.ec2AsgEvidenceMode || "pre-apply" : undefined,
    "--image",
