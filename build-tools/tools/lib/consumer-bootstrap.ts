...
const gitignoreEntries = [".viberoots/", "buck-out/", "projects/config/local.json", ".local/"];
...
async function writeGeneratedFile(
  ...
  if ((await exists(file)) && !(await fsp.readFile(file, "utf8")).includes(generatedMarker)) {
...
function envrc(viberootsUrl: string): string {
  const localOverride =
    viberootsUrl.startsWith("path:") || viberootsUrl.startsWith("git+file:")
      ? ` --override-input viberoots "path:\${VIBEROOTS_SOURCE_ROOT:-\${PWD}/viberoots}"`
      : "";
  return `# ${generatedMarker}
