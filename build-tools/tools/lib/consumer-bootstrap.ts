...
import { installCanonicalArtifactToolsAuthority } from "./artifact-tool-authority";
...
function bootstrapArtifactToolsRootCandidates(): string[] {
...
async function writeBootstrapToolchainAuthority(workspaceRoot: string): Promise<void> {
  for (const candidate of bootstrapArtifactToolsRootCandidates()) {
    let root = "";
    try {
      root = await installCanonicalArtifactToolsAuthority(workspaceRoot, candidate);
    } catch {
      continue;
    }
    const bzl = path.join(
    ...
    await fsp.mkdir(path.dirname(bzl), { recursive: true });
    await writeIfChanged(
      bzl,
      [
        "# GENERATED FILE - DO NOT EDIT.",
        "# Generated by viberoots bootstrap before devshell materialization.",
        `NIX_ARTIFACT_TOOLS_ROOT = ${JSON.stringify(root)}`,
        "",
      ].join("\n"),
    );
    return;
...
export async function initConsumer(opts: InitConsumerOptions): Promise<void> {
  ...
  await writeBootstrapToolchainAuthority(opts.workspaceRoot);
