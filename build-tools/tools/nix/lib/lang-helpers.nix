...
let
  ...
  # Mirrors build-tools/tools/lib/labels.ts:packagePathFromLabel.
  ...
  # - lower-cases
  # - ensures "pkgs." prefix
  # - applies alias mapping from build-tools/tools/lib/nix-attr-aliases.json
  ...
  readDevOverrides = envName:
    let
      ...
      # Preferred local notice lives in the prebuild guard (build-tools/tools/buck/prebuild/notice.ts).
