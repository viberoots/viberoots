...
def _zx_test_impl(ctx):
    ...
    run_and_report = (
        (
            ...
            + "  mkdir -p \"$V8COV_DIR\"; "
            + "  if [ \"$(uname -s 2>/dev/null || true)\" = \"Darwin\" ]; then [ ! -e \"$WORKSPACE_ROOT/buck-out/.metadata_never_index\" ] && : > \"$WORKSPACE_ROOT/buck-out/.metadata_never_index\"; [ ! -e \"$WORKSPACE_ROOT/buck-out/tmp/.metadata_never_index\" ] && : > \"$WORKSPACE_ROOT/buck-out/tmp/.metadata_never_index\"; [ ! -e \"$V8COV_DIR/.metadata_never_index\" ] && : > \"$V8COV_DIR/.metadata_never_index\"; fi; "
            + "fi; "
            + "export BUCK_TEST_TARGET=\"%s\"; "
            + "export TEST_LOG_DIR=\"${TEST_LOG_DIR:-$(pwd)/buck-out/test-logs}\"; "
            + "mkdir -p \"$TEST_LOG_DIR\"; "
            + "if [ \"$(uname -s 2>/dev/null || true)\" = \"Darwin\" ]; then [ ! -e \"$TEST_LOG_DIR/.metadata_never_index\" ] && : > \"$TEST_LOG_DIR/.metadata_never_index\"; fi; "
            ...
            + "if [ \"$(uname -s 2>/dev/null || true)\" = \"Darwin\" ]; then [ ! -e \"$WORKSPACE_ROOT/buck-out/.metadata_never_index\" ] && : > \"$WORKSPACE_ROOT/buck-out/.metadata_never_index\"; [ ! -e \"$TMPDIR/.metadata_never_index\" ] && : > \"$TMPDIR/.metadata_never_index\"; fi; "
            ...
            + "  if [ \"$(uname -s 2>/dev/null || true)\" = \"Darwin\" ]; then [ ! -e \"$WORKSPACE_ROOT/.viberoots/.metadata_never_index\" ] && : > \"$WORKSPACE_ROOT/.viberoots/.metadata_never_index\"; fi; "
            + "  if [ ! -e \"$WORKSPACE_ROOT/.viberoots/current/build-tools/tools/dev/zx-init.mjs\" ]; then rm -f \"$WORKSPACE_ROOT/.viberoots/current\"; fi; "
            + "  if [ ! -e \"$WORKSPACE_ROOT/.viberoots/current\" ]; then "
            + "    if [ -n \"$VBR_ROOT\" ] && [ -f \"$VBR_ROOT/build-tools/tools/dev/zx-init.mjs\" ]; then ln -s \"$VBR_ROOT\" \"$WORKSPACE_ROOT/.viberoots/current\"; "
            + "    elif [ -f \"$WORKSPACE_ROOT/viberoots/build-tools/tools/dev/zx-init.mjs\" ]; then ln -s ../viberoots \"$WORKSPACE_ROOT/.viberoots/current\"; "
            + "    else ln -s .. \"$WORKSPACE_ROOT/.viberoots/current\"; fi; "
            + "  fi; "
            ...
            + "EOF\n"
            + "  mkdir -p \"$WORKSPACE_ROOT/toolchains\" && if [ \"$(uname -s 2>/dev/null || true)\" = \"Darwin\" ]; then [ ! -e \"$WORKSPACE_ROOT/toolchains/.metadata_never_index\" ] && : > \"$WORKSPACE_ROOT/toolchains/.metadata_never_index\"; fi; printf '[buildfile]\nname = TARGETS\n' > \"$WORKSPACE_ROOT/toolchains/.buckconfig\"; "
            + "            fi; "
            ...
            + "if [ -n \"$NODE_V8_COVERAGE\" ]; then mkdir -p \"$NODE_V8_COVERAGE\"; if [ \"$(uname -s 2>/dev/null || true)\" = \"Darwin\" ]; then [ ! -e \"$(dirname \"$NODE_V8_COVERAGE\")/.metadata_never_index\" ] && : > \"$(dirname \"$NODE_V8_COVERAGE\")/.metadata_never_index\"; [ ! -e \"$NODE_V8_COVERAGE/.metadata_never_index\" ] && : > \"$NODE_V8_COVERAGE/.metadata_never_index\"; fi; fi; "
            + "NODE_PATH_ROOT=\"$WORKSPACE_ROOT/node_modules\"; "
            + "if [ ! -d \"$NODE_PATH_ROOT\" ] && [ -d \"$VBR_ROOT/node_modules\" ]; then NODE_PATH_ROOT=\"$VBR_ROOT/node_modules\"; fi; "
            + "if [ -n \"$NODE_PATH\" ]; then export NODE_PATH=\"$NODE_PATH_ROOT:$NODE_PATH\"; else export NODE_PATH=\"$NODE_PATH_ROOT\"; fi; "
            ...
            + "  fi; "
            + "} > \"$OUT_LOG\" 2> \"$RAW_ERR_LOG\"; STATUS=$?; set -e; "
            + "cat \"$OUT_LOG\"; "
            + "grep -Ev 'buck2_client_ctx::file_tailers::tailer: Failed to read from .*buckd\\.(stderr|stdout).*: task [0-9]+ was cancelled' \"$RAW_ERR_LOG\" | tee -a \"$ERR_LOG\" >&2 || true; "
