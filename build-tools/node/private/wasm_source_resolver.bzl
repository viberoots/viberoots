...
def wasm_source_resolver_shell():
    return (
        ...
        + "return 2; "
        + "}; "
        + "validate_node_wasm_file() { "
        + "MACRO=\"$1\"; RAW=\"$2\"; FILE=\"$3\"; "
        + "if [ ! -f \"$FILE\" ]; then echo \"$MACRO: resolved wasm artifact is not a file for '$RAW': $FILE\" >&2; return 2; fi; "
        + "case \"$FILE\" in *.wasm) ;; *) echo \"$MACRO: resolved artifact has the wrong file type for '$RAW': $FILE\" >&2; return 2 ;; esac; "
        + "MAGIC=\"`LC_ALL=C od -An -tx1 -N4 \"$FILE\" 2>/dev/null | tr -d '[:space:]'`\"; "
        + "if [ \"$MAGIC\" != \"0061736d\" ]; then echo \"$MACRO: resolved artifact is not a WebAssembly module for '$RAW': $FILE\" >&2; return 2; fi; "
        + "VBR_WASM_RESOLVED_PATH=\"$FILE\"; return 0; "
        + "}; "
        + "select_node_wasm_file() { "
        + "MACRO=\"$1\"; RAW=\"$2\"; FILE=\"$3\"; ARTIFACT_NAME=\"$4\"; ARTIFACT_GLOB=\"$5\"; "
        + "BASE=\"${FILE##*/}\"; "
        + "if [ -n \"$ARTIFACT_NAME\" ] && [ \"$BASE\" != \"$ARTIFACT_NAME\" ]; then "
        + "echo \"$MACRO: artifact_name '$ARTIFACT_NAME' did not match resolved wasm artifact for '$RAW': $BASE\" >&2; return 2; fi; "
        + "if [ -n \"$ARTIFACT_GLOB\" ]; then case \"$BASE\" in $ARTIFACT_GLOB) ;; "
        + "*) echo \"$MACRO: artifact_glob '$ARTIFACT_GLOB' did not match resolved wasm artifact for '$RAW': $BASE\" >&2; return 2 ;; esac; fi; "
        + "validate_node_wasm_file \"$MACRO\" \"$RAW\" \"$FILE\"; "
        + "}; "
        + "resolve_node_wasm_surface_file() { "
        + "MACRO=\"$1\"; RAW=\"$2\"; SURFACE=\"$3\"; ARTIFACT_NAME=\"$4\"; ARTIFACT_GLOB=\"$5\"; "
        + "SURFACE_KIND=\"`sed -n 's/^kind=//p' \"$SURFACE\"`\"; "
        + "SURFACE_WASM=\"`sed -n 's/^wasm=//p' \"$SURFACE\"`\"; "
        + "SURFACE_ROOT=\"`sed -n 's/^nix_out=//p' \"$SURFACE\"`\"; "
        + "if [ \"$SURFACE_KIND\" != \"emscripten\" ] || [ -z \"$SURFACE_WASM\" ] || [ -z \"$SURFACE_ROOT\" ]; then "
        + "echo \"$MACRO: rejected metadata/manifest output for '$RAW': $SURFACE\" >&2; return 2; fi; "
        ...
        + "if [ -f \"$CAND\" ]; then "
        + "if validate_node_wasm_file \"$MACRO\" \"$RAW\" \"$CAND\" >/dev/null 2>&1; then "
        + "select_node_wasm_file \"$MACRO\" \"$RAW\" \"$CAND\" \"$ARTIFACT_NAME\" \"$ARTIFACT_GLOB\"; return $?; "
        + "fi; "
        + "resolve_node_wasm_surface_file \"$MACRO\" \"$RAW\" \"$CAND\" \"$ARTIFACT_NAME\" \"$ARTIFACT_GLOB\"; return $?; "
        + "fi; "
        + "if [ -n \"$ARTIFACT_NAME\" ]; then "
        + "if [ -d \"$CAND\" ]; then "
        + "NAMED=\"$CAND/$ARTIFACT_NAME\"; "
        + "if [ -f \"$NAMED\" ]; then validate_node_wasm_file \"$MACRO\" \"$RAW\" \"$NAMED\"; return $?; fi; "
        + "fi; "
        ...
        + "done; "
        + "if [ -n \"$NAMED_ONE\" ] && [ -z \"$NAMED_MULTI\" ]; then validate_node_wasm_file \"$MACRO\" \"$RAW\" \"$NAMED_ONE\"; return $?; fi; "
        ...
        + "return 2; "
        + "fi; "
        + "if [ ! -d \"$CAND\" ]; then echo \"$MACRO: wasm artifact surface is neither a reviewed file nor directory for '$RAW': $CAND\" >&2; return 2; fi; "
        + "if [ -z \"$ARTIFACT_GLOB\" ] && [ -f \"$CAND/top.wasm\" ]; then "
        + "validate_node_wasm_file \"$MACRO\" \"$RAW\" \"$CAND/top.wasm\"; return $?; "
        + "fi; "
        ...
        + "done; "
        + "if [ -n \"$FIRST\" ] && [ -z \"$HAS_MULTI\" ]; then validate_node_wasm_file \"$MACRO\" \"$RAW\" \"$FIRST\"; return $?; fi; "
        + "if [ -z \"$FIRST\" ]; then "
        + "if [ -n \"$ARTIFACT_GLOB\" ]; then "
