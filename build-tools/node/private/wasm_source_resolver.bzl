...
def wasm_source_resolver_shell():
    return (
        ...
        + "if [ -n \"$HINT\" ] && [ -e \"$HINT\" ]; then BNX_WASM_RESOLVED_PATH=\"$HINT\"; return 0; fi; "
        + "if [ -n \"$HINT\" ]; then "
        + "HINT_SRCDIR=\"$SRCDIR/$HINT\"; if [ -e \"$HINT_SRCDIR\" ]; then BNX_WASM_RESOLVED_PATH=\"$HINT_SRCDIR\"; return 0; fi; "
        + "HINT_WORKSPACE=\"$WORKSPACE_ROOT/$HINT\"; if [ -e \"$HINT_WORKSPACE\" ]; then BNX_WASM_RESOLVED_PATH=\"$HINT_WORKSPACE\"; return 0; fi; "
        + "HINT_STEM=\"$HINT\"; case \"$HINT_STEM\" in *.wasm) HINT_STEM=\"${HINT_STEM%.wasm}\" ;; esac; "
        + "if [ -n \"$HINT_STEM\" ] && [ -e \"$HINT_STEM\" ]; then BNX_WASM_RESOLVED_PATH=\"$HINT_STEM\"; return 0; fi; "
        + "HINT_STEM_SRCDIR=\"$SRCDIR/$HINT_STEM\"; if [ -e \"$HINT_STEM_SRCDIR\" ]; then BNX_WASM_RESOLVED_PATH=\"$HINT_STEM_SRCDIR\"; return 0; fi; "
        + "HINT_STEM_WORKSPACE=\"$WORKSPACE_ROOT/$HINT_STEM\"; if [ -e \"$HINT_STEM_WORKSPACE\" ]; then BNX_WASM_RESOLVED_PATH=\"$HINT_STEM_WORKSPACE\"; return 0; fi; "
        + "fi; "
        + "C1=\"$HINT\"; C2=\"$SRCDIR/$RAW\"; C3=\"$WORKSPACE_ROOT/$RAW\"; "
        + "if [ -e \"$C1\" ]; then BNX_WASM_RESOLVED_PATH=\"$C1\"; return 0; fi; "
        + "if [ -e \"$C2\" ]; then BNX_WASM_RESOLVED_PATH=\"$C2\"; return 0; fi; "
        + "if [ -e \"$C3\" ]; then BNX_WASM_RESOLVED_PATH=\"$C3\"; return 0; fi; "
        + "TARGET_NAME=\"$RAW\"; "
        ...
        + "if [ -e \"$P\" ]; then BNX_WASM_RESOLVED_PATH=\"$P\"; return 0; fi; "
        + "done; "
        + "MATCH=\"\"; AMBIG=\"\"; "
        ...
        + "done; "
        + "if [ -n \"$MATCH\" ] && [ -z \"$AMBIG\" ]; then BNX_WASM_RESOLVED_PATH=\"$MATCH\"; return 0; fi; "
        + "fi; "
        + "WASM_ONE=\"\"; WASM_MULTI=\"\"; "
        ...
        + "done; "
        + "if [ -n \"$WASM_ONE\" ] && [ -z \"$WASM_MULTI\" ]; then BNX_WASM_RESOLVED_PATH=\"$WASM_ONE\"; return 0; fi; "
        ...
        + "if [ -f \"$CAND\" ]; then BNX_WASM_RESOLVED_PATH=\"$CAND\"; return 0; fi; "
        + "if [ -n \"$ARTIFACT_NAME\" ]; then "
        + "if [ -d \"$CAND\" ]; then "
        + "NAMED=\"$CAND/$ARTIFACT_NAME\"; "
        + "if [ -f \"$NAMED\" ]; then BNX_WASM_RESOLVED_PATH=\"$NAMED\"; return 0; fi; "
        + "fi; "
        ...
        + "done; "
        + "if [ -n \"$NAMED_ONE\" ] && [ -z \"$NAMED_MULTI\" ]; then BNX_WASM_RESOLVED_PATH=\"$NAMED_ONE\"; return 0; fi; "
        ...
        + "return 2; "
        + "fi; "
        + "if [ ! -d \"$CAND\" ]; then BNX_WASM_RESOLVED_PATH=\"$CAND\"; return 0; fi; "
        + "if [ -z \"$ARTIFACT_GLOB\" ] && [ -f \"$CAND/top.wasm\" ]; then "
        + "BNX_WASM_RESOLVED_PATH=\"$CAND/top.wasm\"; return 0; "
        + "fi; "
        ...
        + "done; "
        + "if [ -n \"$FIRST\" ] && [ -z \"$HAS_MULTI\" ]; then BNX_WASM_RESOLVED_PATH=\"$FIRST\"; return 0; fi; "
        + "if [ -z \"$FIRST\" ]; then "
        + "if [ -n \"$ARTIFACT_GLOB\" ]; then "
