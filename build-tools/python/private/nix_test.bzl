...
def _python_nix_test_impl(ctx):
    raw = ctx.attrs.self_label
    expected_bin = "pytest-" + sanitize_name(raw)
    ...
    run_and_exec = (
        ...
        + "if [ ! -x \"$BIN\" ]; then "
        + "  echo '[python_nix_test] expected runner not found:' \"$BIN\" >&2; "
    ...
    command = external_runner_command(
        labels,
        [nix_artifact_bash(), "-c", run_and_exec, "python_nix_test", ctx.attrs._graph_json, ctx.attrs._workspace_root_env] + snapshot_args + declared_inputs,
...
python_nix_test = rule(
    impl = _python_nix_test_impl,
    attrs = _PYTHON_NIX_TEST_ATTRS,
)
