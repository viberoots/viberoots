...
Go cgo consumers can now opt into following a C++ library’s `link_deps` transitively by setting `link_closure` (see `build-tools/docs/go-linking.md`). This uses the same shared link-closure resolver as C++ targets, so traversal order and failure modes are consistent across languages.
