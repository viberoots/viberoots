...
The verify wrapper accepts remote policy only when explicitly requested with `VBR_REMOTE_EXEC_MODE=hybrid|remote|remote-only-conformance`. Remote verify requires absolute `VBR_REMOTE_BUCK_CONFIG` and `VBR_REMOTE_ARTIFACT_DIR` paths plus `VBR_REMOTE_EXEC_SYSTEM=x86_64-linux|aarch64-linux|aarch64-darwin`. Invalid policy fails before local-only verify setup starts. `VBR_REMOTE_TEST_PROFILE_<PASS_NAME>` can override the default `<system-prefix>-default` test profile for a pass; system prefixes are `linux-x86_64`, `linux-aarch64`, and `darwin-aarch64`.

Remote verify passes `--config-file <generated-config>`, `--prefer-remote` for hybrid/remote lanes, `--remote-only` for conformance lanes, and `--unstable-allow-compatible-tests-on-re`. Buck-native artifacts are written below `VBR_REMOTE_ARTIFACT_DIR/<pass>/`: `buck-event-log.pb.zst`, `buck-build-report.json`, `buck-build-id.txt`, `buck-command-report.json`, `test-executor-stdout.log`, and `test-executor-stderr.log`. Failed input/output materialization is disabled by default; set `VBR_REMOTE_MATERIALIZE_FAILED_INPUTS=1` or `VBR_REMOTE_MATERIALIZE_FAILED_OUTPUTS=1` only for debugging runs whose repo-owned metadata is retained under `VBR_REMOTE_ARTIFACT_DIR/<pass>/failed-materialization-policy.json`. The pinned Buck2 flags are bare `--materialize-failed-inputs` and `--materialize-failed-outputs`, so the verify wrapper records retention-scoped metadata under the artifact directory instead of claiming Buck accepts a materialization output path. Verify logs include the selected profile and a redacted generated-config fingerprint such as `config_fingerprint=sha256:0123456789abcdef`; they do not print the config path or secret-bearing config contents.
