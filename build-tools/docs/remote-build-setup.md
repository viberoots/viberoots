...
discover the effective configured substituters from Nix, issue one bounded request for each cache's
`nix-cache-info` endpoint using the reviewed `netrc-file` when configured, and, by default, remove
unreachable optional caches from the per-process `NIX_CONFIG` while keeping required caches, reachable
configured caches, and Nix fallback enabled. Set `VBR_NIX_CACHE_POLICY=strict` only when cache
availability is the behavior under test, or
...
Remote-ready wrappers that reference exact `/nix/store/...` paths must declare a Nix store materialization manifest instead of relying on a warm worker store. The manifest schema is `viberoots.nix-store-materialization.v1` and lists the source revision, declared source snapshot, flake lock fingerprint, substituter endpoint identity, trusted public keys, the Nix tool closure, and each required output path with its attr name, optional NAR hash, and expected output identity. Use `build-tools/lang/nix_store_materialize.bzl` to attach `materialization-manifest:declared` evidence, and use `build-tools/tools/remote-exec/nix-store-materialize.ts --manifest <file> --dry-run` for local conformance checks. Dry-runs cover attrs such as `.#remote-worker-tools`, `.#test-seed`, and selected target outputs without needing live cache credentials. The helper uses `nix copy --from <substituter>` or `nix build <snapshot>#<attr> --no-link --print-out-paths`; it must not install global substituters or write credentials into manifests.
