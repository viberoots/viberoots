...
Artifact construction has one enforced policy authority with three classifications:
...
state is unknown. These classifications are authoritative. Protected purposes fail before
construction or publication when the effective policy cannot be proven.
...
names, substituter URLs, credentials, timestamps, or machine identity. Normal artifact evaluation
is pure and consumes immutable bundle inputs. The public hermeticity claim remains evidence-pending
until the protected three-system, two-builder lane produces and retains its signed aggregate for a
frozen revision.
...
1. **Planner languages vs. macro-only languages:** Go and C++ are “planner languages” (the Nix planner emits derivations for them via templates). Node is handled by Buck macros and importer‑scoped providers; it is built and tested through Nix shims. The Node planner plugin (`build-tools/tools/nix/planner/node.nix`) is used for selected planner builds of `kind:gen`, `kind:lib`, and `kind:bin` artifact paths, and for CLI bundling (`kind:app`) where needed. `nix_node_gen` (and aliases `nix_node_lib`/`nix_node_bin`) use a two-target shape: a planner-visible companion target keeps the original gen command, and the public macro target is a Nix-calling wrapper that builds `graph-generator-selected` for that companion. `node_asset_stage` and `node_wasm_inline_module` use standalone nix-calling genrule route in `build-tools/node/defs_stage.bzl`, and enforce shared command-assembly invariants (`nix_calling_genrule_bootstrap`, `nix_calling_env_export_buck_graph_json`, `nix_build_out_path_cmd`) plus shared wiring (`_prepare_node_nix_calling_genrule`). This does not change provider/auto_map flows or the Node importer‑scoped provider model. The Go planner is split into `build-tools/tools/nix/planner/go.nix` for core Go and `build-tools/tools/nix/planner/go-wasm.nix` for TinyGo Wasm handling.
...
- Node macros that call Nix must use the standardized helper surface in `//build-tools/lang:nix_shell.bzl` for genrule-style command assembly (e.g., `node_webapp`, `node_vercel_next_artifact`, `node_service_artifact`, bundled `nix_node_cli_bin`).
...
- Temporary workspaces live under per-language prefixes in the system temp directory: `$(os.tmpdir)/viberoots-patch-<lang>/...` (e.g., `viberoots-patch-go`, `viberoots-patch-python`, `viberoots-patch-cpp`).
...
- `runnable.artifacts`: referenced artifact paths.
  - Static webapp example: `{ "dist": "<distDir>", "serverWasmContract": "<dist/server/wasm/<default-module>.wasm>" }`
  - SSR webapp example: `{ "serverEntry": "<path>", "clientDir": "<path>", "serverWasmContract": "<dist/server/wasm/<default-module>.wasm>" }`
...
- SSR packaging contract:
  ...
  - Server-side Wasm contract path is manifest-driven and shared across static and SSR outputs: `dist/server/wasm/<default-module>.wasm`.
...
This file is generated by `build-tools/tools/buck/gen-auto-map.ts`. Viberoots-owned Buck macros
load `MODULE_PROVIDERS` from `@workspace_providers//:auto_map.bzl` so provider mappings stay owned
by the consuming workspace. Do not edit by hand.
...
```starlark
load("@viberoots//build-tools/go:defs.bzl", "nix_go_binary", "nix_go_library")
