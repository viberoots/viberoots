...
- Reuse common utilities:
  - Starlark helpers in `build-tools/lang/defs_common.bzl` (`stamp_labels`, `ensure_single_lockfile_label`, `append_nixpkg_labels`, `providers_for`, `append_patch_srcs`).
...
- Writes `.viberoots/workspace/providers/TARGETS.python.auto` deterministically with entries (metadata‑only; patches are not consumed as srcs by the provider rule — macros pull them into target `srcs` for invalidation):
...
- Provider and auto‑map reuse
  ...
  - Glue files (`.viberoots/workspace/buck/graph.json`, `.viberoots/workspace/providers/TARGETS*.auto`, `.viberoots/workspace/providers/auto_map.bzl`) are generated by zx scripts and are not committed.
...
- `.viberoots/workspace/buck/graph.json` shows correct lockfile labels on Python targets.
...
- Emit `.viberoots/workspace/providers/TARGETS.python.auto`:
...
- `build-tools/python/defs.bzl`:
  - Use `stamp_labels`, `ensure_single_lockfile_label`, `append_nixpkg_labels`, and `providers_for` from `build-tools/lang/defs_common.bzl`,
...
- `build-tools/tools/buck/gen-provider-index.ts`:
  - Read Python providers from `.viberoots/workspace/providers/TARGETS.python.auto` (or via `findUvLockfiles`) and emit entries with `kind: "python"` and key `lockfile:<path>#<importer>`.
- Tests:
  - Add zx tests validating Python entries appear, ordering is deterministic, and JSON/BZL outputs match.

#### Acceptance Criteria

- `.viberoots/workspace/providers/provider_index.bzl/json` contain Python entries with correct keys and stable ordering.
...
- Macros stamp `lang:`/`kind:` and wire providers from `auto_map.bzl` using `build-tools/lang/defs_common.bzl` (Go/Node/C++ parity).
