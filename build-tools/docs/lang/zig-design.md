...
- `//third_party/providers/defs_zig.bzl`:
  ...
  - Note: Go does not use provider rules; its patching is package‑local and driven via `srcs` (see build-tools/docs/build-system-design.md). Zig uses provider stamps as described here.
