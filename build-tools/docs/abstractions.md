...
- **Package-local patching** (Go, C++):
  ...
  - Planner-visible stubs for package-local languages still carry package-local patch files as real inputs, and must stamp `patch_scope:package-local`:
    ...
    - For `nix_go_carchive`, I use the package-local wiring helper (`prepare_language_wiring(...)`) and a Nix-backed build rule (`go_nix_build_carchive`) so provider edges are realized in `deps` while patch inputs stay in `srcs`.
