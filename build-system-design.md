...
1. **Planner node inspection helpers:** I keep them in `tools/nix/planner/lib.nix`. I moved rule‑type/label detection and kind inference out of `tools/nix/planner/go.nix`, `tools/nix/planner/cpp.nix`, and `tools/nix/planner/python-core.nix`. Planners call the shared helpers instead of re‑implementing them.
