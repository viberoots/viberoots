...
async function main() {
  ...
  // Post-build housekeeping: safe, low-noise cleanup/optimise/GC
  try {
    ...
    if (!isCI && hkEnabled) {
      ...
      // 1) Always clean ephemeral temp outs (best-effort)
      ...
      // 2) Disk stats
      async function getDiskStats(): Promise<{ freePct: number; freeBytes: number }> {
        try {
          const { stdout } = await $({
            stdio: "pipe",
            cwd: repoRoot(),
          ...
          // POSIX df -Pk: Filesystem 1024-blocks Used Available Use% Mounted_on
      ...
      // 3) Throttled optimise and bounded GC under pressure
      const hkDir = path.join(repoRoot(), "buck-out", ".housekeeping");
      ...
      const { freePct: beforePct, freeBytes: beforeBytes } = await getDiskStats();
      const underPressure = beforePct < 12 || beforeBytes < 8 * 1024 * 1024 * 1024; // <12% or <8GiB
      console.log(
        `[housekeeping] disk status: free=${beforePct.toFixed(0)}% (${fmtBytes(beforeBytes)})`,
      );

      // Optimise: at most once every 2 hours
      ...
      // GC: auto by default; small, escalating cap; min 10 minutes between runs; skip in CI
      if (gcMode === "auto" && underPressure && (await olderThanMinutes(gcStamp, 10))) {
        ...
        await touch(gcStamp);
        // Re-check pressure; escalate next time only if still low
        const { freePct: afterPct, freeBytes: afterBytes } = await getDiskStats();
