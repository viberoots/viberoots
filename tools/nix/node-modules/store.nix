    ...
    in pkgs.stdenvNoCC.mkDerivation {
      ...
      buildPhase = ''
        ...
        if [ ! -f pnpm-lock.yaml ]; then
          ...
          echo "[nix] mkPnpmStoreUnfixed: no lockfile present; seed a lockfile first using tools/dev/update-pnpm-hash.ts --lockfile ${relLock} (set NIX_PNPM_ALLOW_GENERATE=1 for generation)" >&2
          exit 4
        fi
        pnpm config set store-dir "$out/store"
        # Force workspace root to current directory to avoid inheriting repo-root workspace
