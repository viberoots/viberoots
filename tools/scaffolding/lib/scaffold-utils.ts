...
function writeTempJson(obj: any): string {
  ...
  const tmp = require("node:fs").mkdtempSync(require("node:path").join(os.tmpdir(), "scaf-"));
  const p = require("node:path").join(tmp, "answers.json");
  require("fs-extra").writeFileSync(p, JSON.stringify(obj, null, 2), "utf8");
  return p;
}
