...
const ZX_INIT = path.join("tools", "dev", "zx-init.mjs");
const ENFORCE_SCRIPT = path.join("tools", "buck", "enforce-node-deps.ts");
...
export async function warnNodeDepsInLocal(repoRoot: string): Promise<void> {
  ...
  try {
    ...
    console.warn("Fix: node tools/buck/enforce-node-deps.ts --fix");
  }
}
