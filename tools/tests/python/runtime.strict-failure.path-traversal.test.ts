...
test("python runtime: patch with path traversal is rejected (strict mode)", async () => {
  await runInTemp("py-strict-traversal", async (tmp, _$) => {
    const $ = _$
      ? _$
    ...
    const bad = ["--- a/../../outside.txt", "+++ b/../../outside.txt", "@@", "-X", "+Y", ""].join(
      "\n",
    );
    ...
    if (!stderr.includes("unsafe patch path traversal")) {
      console.error("expected strict traversal rejection, got:", stderr);
      process.exit(2);
    }
  });
});
