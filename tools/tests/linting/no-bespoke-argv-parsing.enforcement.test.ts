...
const EXCLUDED_FILES = new Set([
  ...
  "tools/tests/lib/node-run.zx-init.args-and-exit-propagation.test.ts",
  "tools/tests/lib/buck-daemon-reaper.ts",
]);

const EXCLUDED_PREFIXES = ["tools/tests/", "tools/scaffolding/templates/"];

const BANNED_PATTERNS: Array<{ re: RegExp; hint: string }> = [
  {
    re: /\bprocess\.argv\.indexOf\(/,
    hint: "Use tools/lib/cli.ts (getFlagStr/getFlagBool/hasFlag).",
  },
  {
  ...
  },
  {
    ...
    hint: "Use tools/lib/cli.ts (getArgvTokens/getPositionals).",
  },
  {
    re: /\(\s*global(?:This)?\s+as\s+any\s*\)\.argv/,
    hint: "Use tools/lib/cli.ts helpers (they handle zx global argv precedence).",
  },
];
...
async function listToolingFiles(repoRoot: string): Promise<string[]> {
  const out: string[] = [];
  const root = path.join(repoRoot, "tools");
...
test("tooling entrypoints do not hand-roll argv parsing (use tools/lib/cli.ts)", async () => {
  ...
  if (hits.length > 0) {
    ...
    throw new Error(
      [
        ...
        "Use tools/lib/cli.ts for flags and positionals (no direct process.argv/global argv reads).",
        "",
        lines + tail,
      ].join("\n"),
    );
