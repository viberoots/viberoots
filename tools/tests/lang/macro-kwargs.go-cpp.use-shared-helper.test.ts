...
test("go/cpp macros: local_patch_dirs and nixpkg_deps pop logic is centralized in //lang helper", async () => {
  ...
  for (const needle of forbidden) {
    assert.ok(
      ...
      `expected go/defs.bzl to avoid direct ${needle}; rely on prepare_package_local_wiring_v2(...) to extract patch dirs + nixpkg deps`,
    );
    assert.ok(
      !cppDefs.includes(needle),
      `expected cpp/defs.bzl to avoid direct ${needle}; rely on prepare_package_local_wiring_v2(...) to extract patch dirs + nixpkg deps`,
    );
  }

  assert.ok(
    goDefs.includes("prepare_package_local_wiring_v2("),
    "expected go/defs.bzl to call prepare_package_local_wiring_v2(...)",
  );
  assert.ok(
    cppDefs.includes("prepare_package_local_wiring_v2("),
    "expected cpp/defs.bzl to call prepare_package_local_wiring_v2(...)",
  );
});
