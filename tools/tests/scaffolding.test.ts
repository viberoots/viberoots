...
async function runInTemp<T>(name: string, fn: (tmp: string) => Promise<T>): Promise<T> {
  ...
  try { return await fn(tmp); } finally { await fsp.rm(tmp, { recursive: true, force: true }).catch(() => {}); }
}
...
describe('scaffolding', () => {
  ...
  test('smoke: scaf new go lib <name> renders README', async () => {
  ...
  test('e2e: move, update, delete and ls reflects state', async () => {
    await runInTemp('scaf-e2e', async (tmp) => {
      ...
      assert.equal(arr.some((r: any) => String(r.path || '').endsWith('libs/demo-moved')), false);
    });
  });

  test('templates: meta.json and help.md validation pass/fail scenarios', async () => {
    // pass
    await runInTemp('tmpl-validate-pass', async (tmp) => {
      await $({ cwd: tmp })`scaf validate all`;
    });
    ...
    await runInTemp('tmpl-validate-fail1', async (tmp) => {
      ...
      let failed = false;
      try { await $({ cwd: tmp, stdio: 'pipe' })`scaf validate tools/scaffolding/templates/go/lib`; }
      catch { failed = true; }
