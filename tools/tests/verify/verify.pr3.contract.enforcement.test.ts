...
test("verify PR-3 contract: repo-local TMPDIR + coverage gating + disk gate strings present", async () => {
  const tmpRoot = await fsp.readFile("tools/dev/verify/tmp-root.ts", "utf8");
  const coverage = await fsp.readFile("tools/dev/verify/coverage.ts", "utf8");
  const housekeeping = await fsp.readFile("tools/dev/verify/housekeeping.ts", "utf8");
