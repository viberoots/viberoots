...
test("exporter: identical batch reuses cached go-list JSON without rewrite", async () => {
  await runInTemp("exporter-cache-reuse", async (tmp, $) => {
    ...
    })`tools/buck/export-graph.ts --simulate ${sim} --out ${graph} --metrics-out ${metrics1} --cache-dir ${cacheDir}`;
    ...
    await $({
      cwd: tmp,
      env: { ...process.env, FORCE_AUTHORITATIVE: "1" },
    })`tools/buck/export-graph.ts --simulate ${sim} --out ${graph} --metrics-out ${metrics2} --cache-dir ${cacheDir}`;
