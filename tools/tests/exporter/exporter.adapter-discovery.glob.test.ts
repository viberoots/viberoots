...
test("exporter discovers adapters by glob and ignores non-existent", async () => {
  await runInTemp("exporter-discovery", async (tmp, $) => {
    ...
    await fs.copy(
      path.join(process.cwd(), "tools/buck/exporter/types.ts"),
      path.join(tmp, "tools/buck/exporter/types.ts"),
    );
    await fs.copy(
      path.join(process.cwd(), "tools/buck/exporter/lang/contract.ts"),
      path.join(tmp, "tools/buck/exporter/lang/contract.ts"),
    );
