...
test("exporter caches go list results and second run is mostly cache hits", async () => {
  await runInTemp("exporter-cache-hit", async (tmp, $) => {
    ...
    const metrics1 = path.join(tmp, "tools/buck/metrics1.json");
    await $({
      cwd: tmp,
    })`tools/buck/export-graph.ts --simulate ${graph} --out ${graph} --metrics-out ${metrics1}`;
    ...
    const metrics2 = path.join(tmp, "tools/buck/metrics2.json");
    await $({
      cwd: tmp,
    })`tools/buck/export-graph.ts --simulate ${graph} --out ${graph} --metrics-out ${metrics2}`;
