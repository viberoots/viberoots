...
test("nix packages expose per-importer node-modules attr when importer exists", async () => {
  ...
  const attr = importer.replace(/[\/ :]+/g, "_");
  const { stdout: s1 } =
    await $`bash -lc 'test -f ${importer}/pnpm-lock.yaml && echo yes || echo no'`;
  if (String(s1).trim() !== "yes") {
    console.log("skipping: no", `${importer}/pnpm-lock.yaml`);
    return;
  }
  const cmd = `nix eval --raw .#node-modules.${attr}.outPath`;
  const { stdout } = await $({ stdio: "pipe" })`bash -lc ${cmd}`.nothrow();
