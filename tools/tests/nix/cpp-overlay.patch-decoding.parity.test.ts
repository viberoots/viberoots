...
test("cpp overlay patch decoding matches canonical decoder", async () => {
  await runInTemp("nix-cpp-overlay-decode", async (tmp, $) => {
    ...
    const overlaySource = path.join(repoRoot, "tools", "nix", "overlays", "cpp-patches.nix");
    const overlayDest = path.join(tmp, "tools", "nix", "overlays", "cpp-patches.nix");
    ...
    const expr = `
      let
        pkgs = import <nixpkgs> {};
        overlay = import ./tools/nix/overlays/cpp-patches.nix;
        final = pkgs // {
