...
test("go math-api lib builds and go test passes (scaffolded with temp math-core)", async () => {
  await runInTemp("go-math-api", async (tmp, _$) => {
    ...
    await $`bash --noprofile --norc -c 'mkdir -p libs/math-core/include/core libs/math-core/include libs/math-core/src/core libs/math-core/src/cwrapper'`;
    await $`bash --noprofile --norc -c 'cat > libs/math-core/include/addon.h <<"EOF"\n#ifndef MATH_CORE_ADDON_H\n#define MATH_CORE_ADDON_H\n#ifdef __cplusplus\nextern "C" {\n#endif\nint add(int a, int b);\n#ifdef __cplusplus\n}\n#endif\n#endif\nEOF'`;
    await $`bash --noprofile --norc -c 'cat > libs/math-core/include/core/math.h <<"EOF"\n#ifndef MATH_CORE_CORE_MATH_H\n#define MATH_CORE_CORE_MATH_H\nnamespace math_core {\ninline int addInts(int a, int b) { return a + b; }\n}\n#endif\nEOF'`;
    await $`bash --noprofile --norc -c 'cat > libs/math-core/src/core/math.cc <<"EOF"\n#include "../../include/core/math.h"\nnamespace math_core {\nstatic int unused_add(int a, int b) { return a + b; }\n}\nEOF'`;
    await $`bash --noprofile --norc -c 'cat > libs/math-core/src/cwrapper/addon.cc <<"EOF"\n#include "../../include/addon.h"\n#include "../../include/core/math.h"\nextern \"C\" int add(int a, int b) { return math_core::addInts(a, b); }\nEOF'`;
    await $`bash --noprofile --norc -c 'cat > libs/math-core/TARGETS <<"EOF"\nload(\"@prelude//:rules.bzl\", \"cxx_library\")\n\ncxx_library(\n    name = \"lib\",\n    srcs = [\n        \"src/core/math.cc\",\n        \"src/cwrapper/addon.cc\",\n    ],\n    headers = [\n        \"include/addon.h\",\n        \"include/core/math.h\",\n    ],\n    preferred_linkage = \"static\",\n    labels = [\"lang:cpp\", \"kind:lib\"],\n    visibility = [\"PUBLIC\"],\n)\nEOF'`;
