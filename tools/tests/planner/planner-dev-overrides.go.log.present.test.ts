...
test("planner logs dev override presence for Go (non-CI)", async () => {
  await runInTemp("planner-dev-overrides-go", async (tmp, $) => {
    ...
    const res = await $({
      ...
      env: {
        ...process.env,
        // Ensure non-CI and set only Go overrides
        CI: "",
    ...
    if (!log.includes("[planner] dev overrides present: go")) {
