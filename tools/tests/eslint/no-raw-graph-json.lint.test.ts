...
async function importPluginFromWorkspace() {
  const pluginPath = path.join(process.cwd(), "tools", "eslint-plugin-bucknix", "index.js");
...
test("bucknix/no-raw-graph-json flags direct reads and allows allowlisted paths", async () => {
  await runInTemp("eslint-rule-check", async (tmp, $) => {
    ...
    const allowedPath = path.join(tmp, "tools", "buck", "exporter", "ok.ts");
    await fs.outputFile(allowedPath, "// tools/buck/graph.json\nexport const ok = true;\n", "utf8");
