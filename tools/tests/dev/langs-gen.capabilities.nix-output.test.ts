...
test("gen-langs: generates langs.nix from manifest capabilities deterministically", async () => {
  await runInTemp("gen-langs-capabilities", async (tmp, $) => {
    const manifest = {
      languages: [
        ...
        },
        {
          id: "rust",
          displayName: "Rust",
          requiredPaths: ["tools/nix/planner/rust.nix"],
          kinds: ["bin", "lib"],
          templatesDir: "tools/scaffolding/templates/rust",
    ...
    await fs.copy(
      path.join(process.cwd(), "tools/dev/gen-langs.ts"),
      path.join(tmp, "tools/dev/gen-langs.ts"),
    );

    // Run generator
    await $({ cwd: tmp })`node tools/dev/gen-langs.ts`;

    // Verify output
    const outPath = path.join(tmp, "tools/nix/langs.nix");
    const txt = await fs.readFile(outPath, "utf8");
    assert.match(txt, /# tools\/nix\/langs\.nix — GENERATED FILE — DO NOT EDIT\./);
