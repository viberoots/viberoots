...
test("sync-providers-node.ts is delegator-only (no provider internals, no custom flag parsing)", async () => {
  const f = "tools/buck/sync-providers-node.ts";
  ...
  assert(!txt.includes("syncNodeProviders"), `${f} must not call syncNodeProviders directly`);
  assert(
