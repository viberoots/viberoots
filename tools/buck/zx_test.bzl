def _zx_test_impl(ctx):
    ...
    run_and_report = (
        (
            ...
            + "    NODE_V8_COVERAGE=\"$V8COV_DIR\" \"$NODE_BIN\" $TEST_NODE_OPTIONS --test --experimental-strip-types --import \"$WORKSPACE_ROOT/tools/dev/zx-init.mjs\" \"$SCRIPT_PATH\"; "
            + "  else "
            + "    \"$NODE_BIN\" $TEST_NODE_OPTIONS --test --experimental-strip-types --import \"$WORKSPACE_ROOT/tools/dev/zx-init.mjs\" \"$SCRIPT_PATH\"; "
            + "  fi; "
