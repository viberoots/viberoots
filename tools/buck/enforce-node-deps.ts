...
async function main(): Promise<void> {
  ...
  if (missingMaps.length > 0) {
    for (const miss of missingMaps) {
      console.error(
        `ERROR: ${miss.importer}/package.json uses workspace:${miss.name} without mapping in tools/node/workspace-map.json`,
      );
    }
    process.exit(1);
  }
  ...
  console.error("Fix: node tools/buck/enforce-node-deps.ts --fix");
  process.exit(1);
}
